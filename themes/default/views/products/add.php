...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#category').change(function () {
            ...
            $('#modal-loading').hide();
			jQuery("#add_product").removeAttr("disabled");
        });
	...
	function checkValid(){
		if(jQuery("#category").val()=='selected'||jQuery("#category").val()==''){
			alert('Vui lòng chọn danh mục');
			jQuery("#category").focus();
			jQuery("#category").trigger('change');
			return false;
		}
		else if(jQuery("#unit").val()=='selected'||jQuery("#unit").val()==''){
			alert('Vui lòng chọn đơn vị');
			jQuery("#unit").focus();
			jQuery("#unit").trigger('change');
			return false;
		}else{		
			jQuery("#add_product").removeAttr("disabled");
			return true;
...
<div class="box">
	 <?php
		$attrib = array('data-toggle' => 'validator', 'role' => 'form','class'=>'','onsubmit' => 'return checkValid()');
    ...
    <div class="box-headerv2">
        <ul id="myTab" class="nav nav-tabs lhson_add_sp_tab">
					...
					<li class=""><a href="#sizemau" class="tab-grey">Tồn đầu</a></li>
					<!--<li class=""><a href="#nhacungcap" class="tab-grey">Nhà cung cấp</a></li>-->
		...
		<div class="main-task-lhson nhapkho">
			<button type="submit" class="btn btn-primary btncls" name="add_product" id="add_product">
                ...
                <div class="col-md-12 _left_lhson tab-pane fade in" id="overview">
                    <div class="col-md-5">					
						...
						<div class="form-group all">
							...
							<div class="input-group">
								<?php
								$_code_default=(isset($_POST['code']) ? $_POST['code'] : ($product ? $product->code : ''));															
								if($Settings->auto_mavach==1&&$product==NULL){
										//lhson code add 22/01/2018
									$CI =get_instance();
									$CI->load->model('products_model');
									$_code_default=$CI->products_model->getLastProductCode();
									$_code_default=(int)$_code_default+1;	
								}
								?>
								<?= form_input('code', $_code_default, 'class="form-control" id="code"  required="required"') ?>
						...
						<div class="form-group">
							<input type="checkbox" class="checkbox" value="1" name="is_active_tmdt" id="is_active_tmdt" <?= $this->input->post('is_active_tmdt') ? 'checked="checked"' : ''; ?>>
						...
						<div class="form-group all" id="category_select_container">
			                
			                <?php
			                $cat['0'] = lang('select').' '.lang('danh mục (*)');
			                foreach ($categories as $pcat) {
			                    $cat[$pcat->id] = $pcat->name;
			                }
			                echo form_dropdown('parent_category[]', $cat, (isset($_POST['category']) ? $_POST['category'] : '0'), 'class="form-control select" id="parent" place_holder="Chọn danh mục" data-select-id="0" style="width:100%" required="required" onchange="get_subcategories(this.value,0)" ')
			                ?>
			            </div>
						<div class="form-group all">
						...
						<div class="form-group">
							...
							?>
							<?= form_dropdown('unit', $pu, set_value('unit', ($product ? $product->unit : '')), 'class="form-control select" id="unit" style="width:100%;"'); ?>
						</div>
						<div class="form-group">
							<?= lang('default_sale_unit', 'default_sale_unit'); ?>
							<?php $uopts[''] = lang('select_unit_first'); ?>
							<?= form_dropdown('default_sale_unit', $uopts, ($product ? $product->sale_unit : ''), 'class="form-control select" id="default_sale_unit" style="width:100%;"'); ?>
						</div>
						<div class="form-group">
					...
					<div class="col-md-6 col-md-offset-1">	
						...
						<div class="form-group all">
							...
							<?= form_input('baohanh', (isset($_POST['baohanh']) ? $_POST['baohanh'] : ($product ? $this->sma->formatDecimal($product->baohanh) : '')), 'class="form-control tip" id="baohanh"') ?>
						</div>
						<?php }?>
						<div class="form-group standard" style="">
                                    ...
                                    if ($this->input->post('attributes')) {
                                        ...
                                        for ($r = 0; $r <= $a; $r++) {
                                            if (isset($_POST['attr_name'][$r]) && (isset($_POST['attr_warehouse'][$r]) || isset($_POST['attr_quantity'][$r]))) {
                                                echo '<tr class="attr"><td><input type="hidden" name="attr_name[]" value="' . $_POST['attr_name'][$r] . '"><span>' . $_POST['attr_name'][$r] . '</span></td><td class="code text-center"><input type="hidden" name="attr_warehouse[]" value="' . $_POST['attr_warehouse'][$r] . '"><input type="hidden" name="attr_wh_name[]" value="' . $_POST['attr_wh_name'][$r] . '"><span>' . $_POST['attr_wh_name'][$r] . '</span></td><td style="display: none;" class="quantity text-center"><input type="hidden" name="attr_quantity[]" value="' . $_POST['attr_quantity'][$r] . '"><span>' . $_POST['attr_quantity'][$r] . '</span></td><td class="price text-right"><input type="hidden" name="attr_price[]" value="' . $_POST['attr_price'][$r] . '"><span>' . $_POST['attr_price'][$r] . '</span></span></td><td class="text-center"><i class="fa fa-times delAttr"></i></td></tr>';
                                            }
                                        }
                                    } elseif ($product_options) {
                                        foreach ($product_options as $option) {
                                            echo '<tr class="attr"><td><input type="hidden" name="attr_name[]" value="' . $option->name . '"><span>' . $option->name . '</span></td><td class="code text-center"><input type="hidden" name="attr_warehouse[]" value="' . $option->warehouse_id . '"><input type="hidden" name="attr_wh_name[]" value="' . $option->wh_name . '"><span>' . $option->wh_name . '</span></td><td style="display: none;" class="quantity text-center"><input type="hidden" name="attr_quantity[]" value="' . $this->sma->formatQuantity($option->wh_qty) . '"><span>' . $this->sma->formatQuantity($option->wh_qty) . '</span></td><td class="price text-right"><input type="hidden" name="attr_price[]" value="' . $this->sma->formatMoney($option->price) . '"><span>' . $this->sma->formatMoney($option->price) . '</span></span></td><td class="text-center"><i class="fa fa-times delAttr"></i></td></tr>';
                                        }
                                    }
                                    ?></tbody>
                                </table>
                            ...
                            if (!empty($warehouses)) {
                                if ($product) {
                                    ...
                                    foreach ($warehouses as $warehouse) {
                                        ...
                                        echo '<div class="col-md-6 col-sm-6 col-xs-6" style="padding-bottom:15px;">' . $warehouse->name . '<br><div class="form-group">' . form_hidden('wh_' . $warehouse->id, $warehouse->id) . '<label style="line-height:30px;">Tồn kho:</label>'.form_input('wh_qty_' . $warehouse->id, (isset($_POST['wh_qty_' . $warehouse->id]) ? $_POST['wh_qty_' . $warehouse->id] : (isset($warehouse->quantity) ? $warehouse->quantity : '')), 'class="form-control wh" id="wh_qty_' . $warehouse->id . '" placeholder="' . lang('quantity') . '"').'</div><div class="form-group"><label style="line-height:30px;">Giá nhập:</label>'.form_input('wh_price_' . $warehouse->id, '', 'class="form-control wh" id="wh_price_' . $warehouse->id . '" placeholder="' . lang('Giá nhập') . '"') . '</div>';
                                        if ($Settings->racks) {
                                    ...
                                    foreach ($warehouses as $warehouse) {
                                        //$whs[$warehouse->id] = $warehouse->name;
                                        echo '<div class="col-md-6 col-sm-6 col-xs-6" style="padding-bottom:15px;">' . $warehouse->name . '<br><div class="form-group">' . form_hidden('wh_' . $warehouse->id, $warehouse->id) .'<label style="line-height:30px;">Tồn kho:</label>'.form_input('wh_qty_' . $warehouse->id, (isset($_POST['wh_qty_' . $warehouse->id]) ? $_POST['wh_qty_' . $warehouse->id] : ''), 'class="form-control" id="wh_qty_' . $warehouse->id . '" placeholder="' . lang('quantity') . '"').'</div><div class="form-group"><label style="line-height:30px;">Giá nhập:</label>'.form_input('wh_price_' . $warehouse->id, '', 'class="form-control" id="wh_price_' . $warehouse->id . '" placeholder="' . lang('Giá nhập') . '"'). '</div>';
            ...
            for (var i in attrs) {
                if (attrs[i] !== '') {
                    <?php if( ! empty($warehouses)) {
                        foreach ($warehouses as $warehouse) {
                            echo '$(\'#attrTable\').show().append(\'<tr class="attr"><td><input type="hidden" name="attr_name[]" value="\' + attrs[i] + \'"><span>\' + attrs[i] + \'</span></td><td class="code text-center"><input type="hidden" name="attr_warehouse[]" value="'.$warehouse->id.'"><span>'.$warehouse->name.'</span></td><td style="display: none;" class="quantity text-center"><input type="hidden" name="attr_quantity[]" value=""><span>0</span></td><td class="price text-right"><input type="hidden" name="attr_price[]" value="0"><span>0</span></span></td><td class="text-center"><i class="fa fa-times delAttr"></i></td></tr>\');';
                        }
                    } else { ?>
...
<script>
    ...
    function get_subcategories_array(category_id) {
        var categories_array = <?php echo json_encode($allcategories1); ?>;
...
<style type="text/css">
	div#category_select_container .select2-container {
    width: 100%!important;
}	
</style>