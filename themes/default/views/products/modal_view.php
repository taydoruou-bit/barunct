								...
								<div class="table-responsive">
									...
									<tbody>
										<?php foreach ($warehouses as $warehouse) {
											{
												echo '<tr><td>' . $warehouse->name . ' (' . $warehouse->code . ')</td><td><strong>' . $warehouse->quantity . '</strong>' . ($warehouse->rack ? ' (' . $warehouse->rack . ')' : '') . '</td><td><button type="button" class="btn btn-info btn-sm" onclick="OpenModal('.$warehouse->id.',\''.$warehouse->name.'\',this)">Xem</button><div id="myModal'.$warehouse->id.'" class="modal fade" role="dialog">
            ...
            if(xmlhttp.readyState==4&&xmlhttp.status==200)
                {
                    ...
                    var tbl='<table class="table"> <thead><tr><th>Ngày</th><th>Loại</th><th>ĐVT</th><th>Số lượng</th><th>Giá</th><th>+/-</th></tr></thead><tbody>';
                    for(i=0;i<rs.length;i++){
                        ...
                        var flag="+";
                        if(rs[i]['trangthai']=="SALE"||rs[i]['trangthai']=="CHINHKHO-"){
                            flag="-";
                        }
                        
                        tbl+="<tr><td>"+rs[i]['date']+"</td><td>"+rs[i]['trangthai']+"</td><td>"+rs[i]['donvi']+"</td><td>"+formatQuantity(quan)+"</td><td>"+formatMoney(rs[i]['price'])+"</td><td>"+flag+"</td></tr>";
                    }
                    tbl+="</tbody></table>";
                    $('#myModal'+id+" .modal-body").text('');
                    $('#myModal'+id+" .modal-body").append(tbl);
                    
                    $('#myModal').html($('#myModal'+id).html()); 
                }
            };
...
div#multiimages a img {height: 70px!important;width: auto!important;margin: 0 auto;}

a.img-thumbnail.change_img {
    width: 100%;
    height: 80px;
    text-align: center;
}
