...
<?php if ($Owner || $Admin) { ?>
    <ul id="myTab" class="nav nav-tabs">
        ...
        <li class=""><a href="#tondau" class="tab-grey">Tồn đầu</a></li>
...
<div class="tab-content">
                                    ...
                                    <table class="table table-borderless table-striped dfTable table-right-left">
                                        ...
                                        <?php if ($Owner || $Admin) {
                                            ...
                                            echo '<tr><td>' . lang("price") . '</td><td>' . $this->sma->formatMoney($product->price) . '</td></tr>';
											 echo '<tr><td>' . lang("product_gia_si") . '</td><td>' . $this->sma->formatMoney($product->gia_si) . '</td></tr>';
                                                ...
                                                <table
                                                    ...
                                                    <?php foreach ($warehouses as $warehouse) {
                                                        if ($warehouse->quantity != 0) {
                                                            echo '<tr><td>' . $warehouse->name . ' (' . $warehouse->code . ')</td><td><strong>' .$warehouse->quantity. '</strong>' . ($warehouse->rack ? ' (' . $warehouse->rack . ')' : '') . '</td><td><button type="button" class="btn btn-info btn-sm" onclick="OpenModal('.$warehouse->id.',\''.$warehouse->name.'\')">Xem</button><div id="myModal'.$warehouse->id.'" class="modal fade" role="dialog">
													  <div class="modal-dialog">
														...
														<div class="modal-content">
														  <div class="modal-header">
															...
															<h4 class="modal-title">Chi tiết xuất - nhập - tồn</h4>
														  </div>
													...
													function OpenModal(id,name){
														...
														$('#myModal'+id+' .modal-title').text("Xuất - nhập - tồn ["+name+"]");
														//get details ajax
														var url='<?= site_url('products/getDetailXuatNhapTon/?v=1&product='.$product->id.'&warehouse=')?>'+id;
															...
															if(xmlhttp.readyState==4&&xmlhttp.status==200)
																{
																	...
																	var tbl='<table class="table"> <thead><tr><th>Số lượng</th><th>Ngày</th><th>Loại</th><th>+/-</th></tr></thead><tbody>';
																	for(i=0;i<rs.length;i++){
																		var quan=rs[i]['quantity'];
																		if(rs[i]['unit_quantity']>0&&rs[i]['unit_quantity']>rs[i]['quantity']){
																			quan=rs[i]['unit_quantity'];
																		}
																		if(rs[i]['unit_quantity']<0&&rs[i]['unit_quantity']<rs[i]['quantity']){
																			quan=rs[i]['unit_quantity'];
																		}
																		var flag="+";
																		if(rs[i]['trangthai']=="SALE"){
																			flag="-";
																		}
																		tbl+="<tr><td>"+quan+"</td><td>"+rs[i]['date']+"</td><td>"+rs[i]['trangthai']+"</td><td>"+flag+"</td></tr>";
																	}
																	tbl+="</tbody></table>";
																	$('#myModal'+id+" .modal-body").text('');
																	$('#myModal'+id+" .modal-body").append(tbl);
																}
															};
																console.log(url);
	...
	<!-- module ton dau -->
	<div id="tondau" class="tab-pane fade">
        <script type="text/javascript">
            $(document).ready(function () {
                oTable = $('#PoTonDau').dataTable({
                    "aaSorting": [[0, "desc"]],
                    "aLengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "<?= lang('all') ?>"]],
                    "iDisplayLength": <?= $Settings->rows_per_page ?>,
                    'bProcessing': true, 'bServerSide': true,
                    'sAjaxSource': '<?= site_url('reports/getTonDauReport/?v=1&product=' . $product->id) ?>',
                    'fnServerData': function (sSource, aoData, fnCallback) {
        ...
        <div class="box">
            <div class="box-header">
                <h2 class="blue"><i class="fa-fw fa fa-star nb"></i><?= $product->name . ' ' . lang('Tồn kho đầu kì'); ?>
                </h2>
            </div>
            <div class="box-content">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="table-responsive">
                            <table id="PoTonDau" class="table table-bordered table-hover table-striped table-condensed">
                                <thead>
                                <tr>
                                    <th><?= lang("date"); ?></th>
    ...
    <div id="purchases" class="tab-pane fade">
        <script type="text/javascript">
            $(document).ready(function () {
				...
				{column_number: 5, filter_default_label: "[<?=lang('nhân viên');?>]", filter_type: "text", data: []},
	...
	<div id="returns" class="tab-pane fade">
        ...
        <script>
			$(document).ready(function () {
				oTable = $('#REData').dataTable({
					...
					"iDisplayLength": <?=$Settings->rows_per_page?>,
					'bProcessing': true, 'bServerSide': true,
					'sAjaxSource': '<?=site_url('returns/getReturnsByProduct?v=1&product=' . $product->id); ?>',
					'fnServerData': function (sSource, aoData, fnCallback) {
						aoData.push({
							"name": "<?=$this->security->get_csrf_token_name()?>",
							"value": "<?=$this->security->get_csrf_hash()?>"
					...
					"aoColumns": [{"bSortable": false,"mRender": checkbox}, {"mRender": fld}, null, null, null,null, {"mRender": currencyFormat}, {"bSortable": false,"mRender": attachment}],
					"fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
						var gtotal = 0;
						for (var i = 0; i < aaData.length; i++) {
        ...
        <div class="box">
            <div class="box-header">
				<h2 class="blue"><i
						class="fa-fw fa fa-random"></i><?php echo ' Thu hồi ';?>
				</h2>
				...
				<div class="box-icon">
					<ul class="btn-tasks">           
						...
						<div class="main-task-lhson">
							<a href="#" class="btn btn-primary btncls" id="excel" data-action="export_excel">
