...
<script type="text/javascript">
    $(document).ready(function () {
		...
		$('#price').number(true,<?=$Settings->decimals?>);
		$('#cost').number(true,<?=$Settings->decimals?>);
		$('#gia_si').number(true,<?=$Settings->decimals?>);
...
<div class="box">
	...
	?>
	<div class="box-headerv2">
        <ul id="myTab" class="nav nav-tabs lhson_add_sp_tab">
					<li class=""><a href="#overview" class="tab-grey">Tổng quan</a></li>
					<li class=""><a href="#sizemau" class="tab-grey standard">Tồn đầu</a></li>
					<!--<li class=""><a href="#nhacungcap" class="tab-grey standard">Nhà cung cấp</a></li>-->
					<li class=""><a href="#chitiet" class="tab-grey">Chi tiết</a></li>
					<li class=""><a href="#tuychinh" class="tab-grey">Tùy chỉnh</a></li>
		</ul>
		<div class="main-task-lhson nhapkho">
			<button type="submit" class="btn btn-primary btncls" name="edit_product" id="edit_product">
				<i class="fa fa-save"></i>
				<?php echo lang('edit_product');?>
			</button>
		</div>
    </div>
    <div class="box-content _sanpham_add">
        <div class="row">
            <div class="col-lg-12 tab-content">
                <div class="col-md-12 _left_lhson tab-pane fade in" id="overview">
					<div class="col-md-5">
					...
					$opts = array('standard' => lang('standard'),'service' => lang('service'));		
						...
						?>
							<div class="form-group all">
								...
								echo form_dropdown('xuatxu', $_xt, (isset($_POST['xuatxu']) ? $_POST['xuatxu'] : ($product ? $product->xuatxu : '')), 'class="form-control select" id="xuatxu" placeholder="' . lang("select") . " " . lang("xuatxu") . '" style="width:100%"')
								?>
							</div>
						<?php }?>
						<?php if ($Settings->is_active_tmdt==1) { ?>
						<div class="form-group">
							<input type="checkbox" class="checkbox" value="1" name="is_active_tmdt" id="is_active_tmdt" <?= $product->is_active_tmdt ? 'checked="checked"' : ''; ?>>
							<label for="is_active_tmdt" class="padding05">
								<?= lang('Đăng lên sàn TMĐT'); ?>
							</label>
						</div>
						<?php
						}
						...
						<div class="form-group" id="category_select_container">			               
			                <?php
			                if (isset($parent_categories_array)&&count($parent_categories_array)) 
			                {
			                    ...
			                    foreach ($parent_categories_array as $cat) 
			                    {
		                            ...
		                            ?>
		                            <select name="parent_category[]" id="parent-<?= $cat->id; ?>" class="form-control select skip" data-select-id="<?= $cat->id; ?>" onchange="get_subcategories(this.value, '<?= $cat->id; ?>');">
			                ...
			                {
			                    $cat[''] = lang('select').' '.lang('danh mục (*)');

			                    foreach ($categories as $pcat) {
			                        $cat[$pcat->id] = $pcat->name;
			                    }
			                    echo form_dropdown('parent_category[]', $cat,$product->category_id, 'class="form-control select skip" id="parent" style="width:100%" onchange="get_subcategories(this.value,'.(int)$product->category_id.')"');
			                }			           
			                ?>
			            </div>
						<div class="form-group all">
							...
							<?php
							$nhom_sp[''] = "";
							foreach ($nhom as $n_sp) {
								$nhom_sp[$n_sp->id] = $n_sp->name;
							}
							echo form_dropdown('group_id', $nhom_sp, (isset($_POST['group_id']) ? $_POST['group_id'] : ($product ? $product->group_id : '')), 'class="form-control select" id="group_id" placeholder="' . lang("select") . " " . lang("Nhóm sản phẩm") . '" style="width:100%"')
							?>
						</div>
					...
					<div class="col-md-6 col-md-offset-1">	
						...
						<div class="form-group standard" style="">
							...
							<input type="text" name="cost" value="<?php echo (isset($_POST['cost']) ? $_POST['cost'] : ($product ? $this->sma->formatDecimal($product->cost) : ''))?>" class="form-control tip" id="cost" data-original-title="" title="">
						</div>
						<div class="form-group all">
						...
						<?php 
						if($Settings->use_gia_si){
						?>
						<div class="form-group all">
							<?= lang("product_gia_si", "gia_si") ?>
							<?= form_input('gia_si', (isset($_POST['gia_si']) ? $_POST['gia_si'] : ($product ? $this->sma->formatDecimal($product->gia_si) : '')), 'class="form-control tip" id="gia_si"') ?>
						</div>
						<?php 
						}
						if($baohanh){
						?>
						<div class="form-group all">
							<?= lang("baohanh", "baohanh") ?>
							<?= form_input('baohanh', (isset($_POST['baohanh']) ? $_POST['baohanh'] : ($product ? $product->baohanh : '')), 'class="form-control tip" id="baohanh"') ?>
						</div>
						<?php }?>
						<div class="form-group standard" style="">
							<label for="weight">Cân nặng (gram)</label>							
							<input type="text" name="weight" value="<?php echo (isset($_POST['weight']) ? $_POST['weight'] : ($product ? $product->weight : ''))?>" class="form-control tip" id="weight" data-original-title="" title="">
						</div>
						...
						<?php
						 ...
						 if ($Settings->khuyenmai) { ?>
						<div class="form-group">
							<input type="checkbox" class="checkbox" value="1" name="promotion" id="promotion" <?= $product->promotion ? 'checked="checked"' : ''; ?>>
				...
				<div id="sizemau" class="col-md-12 _right_lhson tab-pane fade">
						...
						<div class="standard">
							<div>
								...
								if ((!empty($warehouses) || !empty($warehouses_products))) {
									...
									echo '<p><strong>'.lang("Tồn đầu").'</strong></p>';
									if (!empty($warehouses_products)) {
										...
										*/	
											echo '<div class="col-md-6 col-sm-6 col-xs-6" style="padding-bottom:15px;">' . $wh_pr->name . '<br><div class="form-group">' . form_hidden('vwh_qty_' . $wh_pr->id, $wh_pr->id) . '<label style="line-height:30px;">Tồn kho:</label>'.form_input('rwh_qty_' . $wh_pr->id, (isset($_POST['rwh_qty_' . $wh_pr->id]) ? $_POST['rwh_qty_' . $wh_pr->id] : (isset($wh_pr->tondau) ? $wh_pr->tondau : '')), 'class="form-control" id="rwh_qty_' . $wh_pr->id . '" placeholder="' . lang('quantity') . '"').'</div><div class="form-group"><label style="line-height:30px;">Giá nhập:</label>'.form_input('rwh_price_' . $wh_pr->id, (isset($_POST['rwh_price_' . $wh_pr->id]) ? $_POST['rwh_price_' . $wh_pr->id] : (isset($wh_pr->giatondau) ? $wh_pr->giatondau : '')), 'class="form-control" id="rwh_price_' . $wh_pr->id . '" placeholder="' . lang('Giá nhập') . '"') . '</div>';
											if ($Settings->racks) {
												echo '<div class="form-group">' . form_input('rrack_' . $wh_pr->id, (isset($_POST['rrack_' . $wh_pr->id]) ? $_POST['rrack_' . $wh_pr->id] : (isset($wh_pr->rack) ? $wh_pr->rack : '')), 'class="form-control " id="rrack_' . $wh_pr->id . '" placeholder="' . lang('rack') . '"') . '</div>';
											}
							...
							<div class="well well-sm">
								...
								<!--<table class="table table-bordered table-condensed table-striped"
									...
									foreach ($product_options as $option) {
										echo '<tr><td class="col-xs-3"><input type="hidden" name="attr_id[]" value="' . $option->id . '"><span>' . $option->name . '</span></td><td class="code text-center col-xs-3"><span>' . $option->wh_name . '</span></td><td class="quantity text-center col-xs-2"><span>' . $this->sma->formatQuantity($option->tondau) . '</span></td><td class="price text-right col-xs-2">' . $this->sma->formatMoney($option->price) . '</td></tr>';
									}
								?>
								</tbody>
								</table>-->
								...
								if ($product_variants) { ?>
									...
									<tr class="active">
										...
										<th class="col-xs-1"><?= lang('price_addition') ?></th>										
										<?php 
											$check=array();
											foreach ($product_variants as $pv) {
												foreach ($product_options as $option) {		
													
													if($option->id==$pv->id&&!$check[$option->wh_id]){
														echo '<th style="display:none" class="col-xs-">Tồn '.$option->wh_name.'</th>';
														$check[$option->wh_id]=$option->wh_id;
													}
									...
									foreach ($product_variants as $pv) {
										 ...
										 echo '<tr id="_tr_xoa'.$pv->id.'"><td class="col-xs-2"><input type="hidden" name="variant_id_' . $pv->id . '" value="' . $pv->id . '"><input type="text" name="variant_name_' . $pv->id . '" value="' . $pv->name . '" class="form-control"></td><td class="price text-right col-xs-2"><input type="text" name="variant_price_' . $pv->id . '" value="' . $pv->price . '" class="form-control"></td>';
										 
										foreach ($product_options as $option) {		
											if($option->id==$pv->id){
												$_tondau=$option->tondau;
												echo '<td style="display:none" class="col-xs-"><input type="text" name="variant_quantity_' . $pv->id.'_'.$option->wh_id . '" value="' . $_tondau . '" class="form-control"></td>';
											}
										}										
										 echo '<td class="text-center col-xs-1"><i attrid="'.$pv->id.'" class="fa fa-times delAttrLhson"></i></td></tr>';
									}
									?>
									</tbody>
									</table>
										...
										<table id="attrTable" class="table table-bordered table-condensed table-striped" style="margin-bottom: 0; margin-top: 10px;">
											<thead>
												<tr class="active">
													<th><?= lang('name') ?></th>
													<th><?= lang('warehouse') ?></th>
													<th style="display: none;"><?= lang('quantity') ?></th>
													<th><?= lang('price_addition') ?></th>
													<th class="text-center"><i class="fa fa-times attr-remove-all"></i></th>
												</tr>
											</thead>
											<tbody><?php
												if ($this->input->post('attributes')) {
													...
													for ($r = 0; $r <= $a; $r++) {
														...
														if (isset($_POST['attr_name'][$r]) && (isset($_POST['attr_warehouse'][$r]) || isset($_POST['attr_quantity'][$r]))) 
														{
															...
															<td class="quantity text-center" style="display: none;"><input type="hidden" name="attr_quantity[]" value="' . $_POST['attr_quantity'][$r] . '"><span>' . $_POST['attr_quantity'][$r] . '</span></td>
                ...
                <div id="nhacungcap" class="col-md-12 tab-pane fade">	
					<div class="col-md-12">
						<div class="form-group standard">
							<div class="form-group">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#addAttributes').click(function (e) {
            ...
            for (var i in attrs) {
                if (attrs[i] !== '') {
                    $('#attrTable').show().append('<tr class="attr"><td><input type="hidden" name="attr_name[]" value="' + attrs[i] + '"><span>' + attrs[i] + '</span></td><td class="code text-center"><input type="hidden" name="attr_warehouse[]" value=""><span></span></td><td style="display:none" class="quantity text-center"><input type="hidden" name="attr_quantity[]" value=""><span></span></td><td class="price text-right"><input type="hidden" name="attr_price[]" value="0"><span>0</span></span></td><td class="text-center"><i class="fa fa-times delAttr"></i></td></tr>');
                }
            }
        });
	    ...
	    $(document).on('click', '.delAttrLhson', function () {
			var id=$(this).attr('attrid');
			// tien hanh xoa bang aJax
			...
			var a = '<?= site_url('products/delAttrLhson/?id=') ?>'+ id;
...
<script>
	$(document).ready(function() {
	    ...
	    $('#is_active_tmdt').on('ifChecked', function (e) {
	    	$('#category_select_container').html('');
	        get_defaultcate();
	    });
	    $('#is_active_tmdt').on('ifUnchecked', function (e) {
	    	$('#category_select_container').html('');
	        get_defaultcate();
	    });
	});
    function get_defaultcate() {

        var subcategories = get_subcategories_array(0);
        
        var date = new Date();       
        if (subcategories.length > 0) {
            var new_data_select_id = date.getTime();
            var select_tag = '<select class="form-control subcategory-select" data-select-id="' + new_data_select_id + '" name="parent_category[]" onchange="get_subcategories(this.value,' + new_data_select_id + ');">' +
                '<option value="">Chọn danh mục</option>';
    ...
    function get_subcategories(category_id, data_select_id) {
        ...
        if (subcategories.length > 0) {
            var new_data_select_id = date.getTime();
            var select_tag = '<select class="form-control custom-select" name="parent_category[]" data-select-id="' + new_data_select_id + '" onchange="get_subcategories(this.value,' + new_data_select_id + ');">' +
    ...
    function get_subcategories_array(category_id) {
        ...
        var categories_array2 = <?php echo json_encode($allcategories2); ?>;
        if($("#is_active_tmdt:checked").length==1){
        	categories_array=categories_array2;
        }
    ...
    $("#is_active_tmdt").change(function(){
    	 $('#category_select_container select').each(function () {
            if (parseInt($(this).attr('data-select-id')) > 0) {                
                $(this).remove();                
            }
        });
    });
