        ...
        <div class="row">
            <div class="col-lg-12">
                <?php if (!empty($inv->return_purchase_ref) && $inv->return_id) {
                    echo '<div class="alert alert-info no-print"><p>Đơn hàng trả ncc: '.$inv->return_purchase_ref;
                ...
                <div class="well well-sm">
                    ...
                    <div class="col-xs-4 border-right">
                        ...
                        <div class="col-xs-10">
                            ...
                            ?>
							<p style="font-weight:bold;">Tổng nợ: 
						<?php 
						//load tong no cu lhson 22/01/2018
						$CI =get_instance();
						$CI->load->model('reports_model');
						$_dathanhtoan= $CI->reports_model->getPurchasesTotals($supplier->id);
						
						$_dathanhtoanlhson= $CI->reports_model->getPurchasesTotalsLhson($supplier->id);
						...
						$company_details = $CI->companies_model->getCompanyByID($supplier->id);		
                    ...
                    <table class="table table-bordered table-hover table-striped print-table order-table">
                        ...
                        <tr>
                            ...
                            if ($Settings->product_discount != 0 && $inv->product_discount != 0) {
							 ...
							 if ($Settings->chiec_khau != 0) {
                                echo '<th style="padding-right:20px; text-align:center; vertical-align:middle;"> CK %</th>';
                            }
                            ?>
                        ...
                        foreach ($rows as $row):
                            ...
                            <tr>
                                ...
                                if ($Settings->product_discount != 0 && $inv->product_discount != 0) {
								 ...
								 if ($Settings->chiec_khau != 0) {
                                    echo '<td style="width: 120px; text-align:right; vertical-align:middle;">' . ($row->chiec_khau != 0 ? '<small>('.$row->chiec_khau.')</small>' : '') . ' ' . $this->sma->formatMoney($row->items_chiec_khau) . '</td>';
                                }
                                ?>
                        ...
                        ?>
						<?php if ($inv->chiec_khau != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("chiec_khau") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">'.($inv->chiec_khau_id ? '<small>('.$inv->chiec_khau_id.')</small> ' : '') . $this->sma->formatMoney($return_purchase ? ($inv->order_discount+$return_purchase->order_discount+$inv->chiec_khau) : $inv->chiec_khau) . '</td></tr>';
                        }
                        ?>
            ...
            <div class="buttons">
                ...
                <div class="btn-group btn-group-justified">
                    ...
                    <div class="btn-group">
					 ...
					 <div class="btn-group">
                        <a href="<?= site_url('purchases/printnhap/' . $inv->id) ?>" data-toggle="modal" data-target="#myModal" class="tip btn btn-warning tip" title="<?= lang('print') ?>">
