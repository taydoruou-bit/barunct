...
<script>
    $(document).ready(function () {
        oTable = $('#POData').dataTable({
            ...
            'sAjaxSource': '<?=site_url('purchases/getPurchasesTraHang' . ($warehouse_id ? '/' . $warehouse_id : ''))?>',
			...
			{column_number: 4, filter_default_label: "[<?=lang('ĐVGH');?>]", filter_type: "text", data: []},
		...
		jQuery("#dtFilter-filter-wrapper--POData-8").prepend(_lockq);
		
		jQuery("#_lockq").on("change", function(value){
			var This = jQuery(this);
			var selectedD = jQuery(this).val();
			jQuery("#dtFilter-filter--POData-8").val(selectedD);
			jQuery("#dtFilter-filter--POData-8").trigger("keyup");
		});
    });
function trahangfunc(){
		...
		jQuery("#POData tbody tr").each(function(){
		...
		if(count==0){
			alert("Bạn chưa chọn hóa đơn cần trả hàng");
			return false;
		}else if(count>1){
			alert("Chỉ trả 1 hóa cùng lúc, bạn đang chọn "+count);
			return false;
		}else{
			if(val==0){
				alert("Không nhận được id hóa đơn");
				return false;
			}else{
				window.location="<?php echo site_url('purchases/return_purchase_ncc/');?>"+val;
			}
		}
	}
</script>
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-star"></i><?=lang('Trả hàng NCC') . ' (' . ($warehouse_id ? $warehouse->name : lang('all_warehouses')) . ')';?>
        </h2>
		...
		<div class="main-task-lhson">
			...
			<a class="btn btn-primary btncls" href="<?=site_url('purchases/add')?>">
				<i class="fa fa-plus-circle"></i> Thêm nhập kho
			</a>
			...
			<a class="btn btn-primary btncls" href="#" id="combine" data-action="combine">
				<i class="fa fa-file-pdf-o"></i> HĐ Pdf
			</a>
			<a class="btn btn-primary btncls bpo" href="#" title="<b><?=lang("delete_purchases")?></b>"
            ...
            <div class="col-lg-12">
                ...
                <div class="table-responsive">
                    <table id="POData" cellpadding="0" cellspacing="0" border="0"
                           class="table table-bordered table-hover table-striped no-print">
                        <thead>
                        <tr class="active">
...
<div id="locketqua" class="form-group">
		<?php
		$_lockq = array('' => lang('All'), 'paid' => lang('paid'), 'partial' => lang('partial'), 'pending' => lang('pending'));
