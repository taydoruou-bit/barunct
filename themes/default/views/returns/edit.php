    ...
    $(document).ready(function () {
        ...
        $("#add_item").autocomplete({
            source: function (request, response) {
                $.ajax({
                    type: 'get',
                    url: '<?= site_url('returns/suggestions'); ?>',
                    dataType: "json",
		...
		if(1==<?php echo (int)$inv->doitac>0?1:0;?>){
			
			jQuery("#div_doitac_id").css("display","block");
			
			
			
			var $doitac=jQuery("#doitac");
			
			localStorage.setItem('doitac', '<?= $inv->doitac ?>');
			doitac = localStorage.getItem('doitac');
			$doitac.val(doitac).select2({
				minimumInputLength: 1,
				data: [],
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-edit"></i>Cập nhật thu hồi</h2>
    </div>
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo form_open_multipart("returns/edit/".$id, $attrib);
                ?>
                <div class="row">
                    <div class="col-lg-12">
                        <?php if ($Owner || $Admin) { ?>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <?php
                                    echo form_input('customer', (isset($_POST['customer']) ? $_POST['customer'] : ""), 'id="recustomer" data-placeholder="' . lang("select") . ' ' . lang("customer") . '" class="form-control ssr-customer input-tip" style="width:100%;"');
                                ?>
                            </div>
                        </div>
						...
						<div class="col-md-4">
								<div class="form-group" id="div_doitac_id">
									<?= lang("Đối tác giao hàng", "doitac"); ?>
										...
										<?php
										echo form_input('doitac', (isset($_POST['doitac']) ? $_POST['doitac'] : $inv->doitac), 'id="doitac" data-placeholder="' . lang("select") . ' ' . lang("đối tác") . '" class="form-control input-tip" style="width:100%;"');
										?>
								</div>
							</div>
                        <div class="col-md-4">
