...
<script>
    $(document).ready(function () {
        ...
        function price_input(x,price) {
            ti = ti+1;
            var v = x.split('__');
           
            return "<div class=\"text-center\"><input placeholder=\""+formatMoney(price)+"\" type=\"text\" name=\"price"+v[0]+"\" value=\""+(v[1] != '' ? formatDecimals(v[1]) : '')+"\" class=\"form-control text-center price\" tabindex=\""+(ti)+"\" style=\"padding:2px;height:auto;\"></div>"; // onclick=\"this.select();\"
        }
        function price_lech(x,price) {
            ti = ti+1;
            var v = x.split('__');
                       
            var lech=getNumber(v[1])-getNumber(price);
           return formatDecimals(lech);
        ...
        $('#CGData').dataTable({
            ...
            "aoColumns": [{"bSortable": false, "mRender": checkbox}, null, null,{'mRender':formatNumber}, {
                "mRender": function ( data, type, row ) {
                    return price_input(row[4],row[3]);
                },
                "aTargets": [ 4 ],"bSearchable": false
            },{
                "mRender": function ( data, type, row ) {
                    return price_lech(row[4],row[3]);
                },
                "aTargets": [ 4 ],"bSearchable": false
            }, {"bSortable": false}]
        }).fnSetFilteringDelay();
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-building"></i>Nhóm giá từng sản phẩm  (<?= $price_group->name; ?>)</h2>
