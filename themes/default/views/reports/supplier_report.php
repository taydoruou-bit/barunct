...
<div class="row">
    <div class="col-sm-12">
        <div class="row">
            <div class="col-sm-6">
                ...
                <div class="small-box padding1010 col-sm-4 borange">
                    <h3><?= (isset($purchases->total_amount) || isset($purchases->paid)) ? $this->sma->formatMoney($purchases->total_amount - $purchases->paid + $nobandau) : $this->sma->formatMoney($nobandau); ?></h3>
					...
					<div class="col-sm-4">
                        <div class="small-box padding1010 borange">
                            <div class="inner clearfix">
                                <a>
                                    <h3><?= $this->sma->formatMoney($nobandau) ?></h3>
		...
		<div class="row">
            <div class="col-sm-6">
                <div class="small-box padding1010 col-sm-4 bblue">
                    <h3><?= isset($returned->total_amount) ? $this->sma->formatMoney($returned->total_amount) : '0.00' ?></h3>

                    <p>Tổng tiền trả</p>
                </div>
                <div class="small-box padding1010 col-sm-4 blightOrange">
                    <h3><?= isset($returned->paid) ? $this->sma->formatMoney($returned->paid) : '0.00' ?></h3>

                    <p>Đã thanh toán trả</p>
                </div>
                <div class="small-box padding1010 col-sm-4 borange">
                    <h3><?= (isset($returned->total_amount) || isset($returned->paid)) ? $this->sma->formatMoney($returned->total_amount - $returned->paid) : 0; ?></h3>
...
<ul id="myTab" class="nav nav-tabs no-print">
	...
	<li class=""><a href="#purcahses-re" class="tab-grey">Trả hàng NCC</a></li>
...
<div id="purcahses-re" class="tab-pane fade in">
        <?php
        $v = "&supplier=" . $user_id;
        if ($this->input->post('submit_return_report')) {
            if ($this->input->post('biller')) {
                $v .= "&biller=" . $this->input->post('biller');
            }
            if ($this->input->post('warehouse')) {
        ...
        <script>
        $(document).ready(function () {
            oTable = $('#PoRDataRe').dataTable({
            "aaSorting": [[0, "desc"]],
            "aLengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "<?= lang('all') ?>"]],
            "iDisplayLength": <?= $Settings->rows_per_page ?>,
            'bProcessing': true, 'bServerSide': true,
            'sAjaxSource': '<?= site_url('reports/getPurchasesReturnLhsonReport/?v=1' . $v) ?>',
            'fnServerData': function (sSource, aoData, fnCallback) {
                aoData.push({
                    "name": "<?= $this->security->get_csrf_token_name() ?>",
                    "value": "<?= $this->security->get_csrf_hash() ?>"
        ...
        <div class="box purchases-table">
            <div class="box-header">
                <h2 class="blue"><i class="fa-fw fa fa-star nb"></i> Báo cáo trả hàng <?php
                if ($this->input->post('start_date')) {
                    echo "From " . $this->input->post('start_date') . " to " . $this->input->post('end_date');
                }
                ?></h2>
...
$(document).ready(function () {
	...
	$('#pdfre').click(function (event) {
        event.preventDefault();
        window.location.href = "<?=site_url('reports/getPurchasesReturnLhsonReport/pdf/?v=1'.$v)?>";
        return false;
    });
    $('#xlsre').click(function (event) {
        event.preventDefault();
        window.location.href = "<?=site_url('reports/getPurchasesReturnLhsonReport/0/xls/?v=1'.$v)?>";
        return false;
    });
