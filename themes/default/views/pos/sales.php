    ...
    $(document).ready(function () {
        oTable = $('#POSData').dataTable({
            ...
            "aoColumns": [{
                "bSortable": false,
                "mRender": checkbox
            }, {"mRender": fld}, null,null,null, null, null, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": row_status}, {"mRender": pay_status}, {"bSortable": false}],
		...
		jQuery("#dtFilter-filter-wrapper--POSData-9").prepend(_lockq);
		
		jQuery("#_lockq").on("change", function(value){
			var This = jQuery(this);
			var selectedD = jQuery(this).val();
			jQuery("#dtFilter-filter--POSData-9").val(selectedD);
			jQuery("#dtFilter-filter--POSData-9").trigger("keyup");
		});
    });
function trahangfunc(){
		var val=0;
		var count=0;
		jQuery("#POSData tbody tr").each(function(){
			var chk=jQuery(this).find("input.checkbox.multi-select.input-xs").is(':checked');
			val=jQuery(this).find("input.checkbox.multi-select.input-xs").val();
...
<div class="box no-print">
    <div class="box-header">
		...
		<div class="main-task-lhson">
			<a class="btn btn-primary btncls" href="<?= site_url('pos') ?>"><i class="fa fa-plus-circle"></i> <?= lang('add_sale') ?></a>
			<a class="btn btn-primary btncls" href="#" id="excel" data-action="export_excel"><i class="fa fa-file-excel-o"></i> Excel</a>
			<a class="btn btn-primary btncls" href="#" id="pdf" data-action="export_pdf"><i class="fa fa-file-pdf-o"></i> Pdf</a>
			<a class="btn btn-primary btncls bpo" href="#"title="<b><?= $this->lang->line("delete_sales") ?></b>" data-content="<p><?= lang('r_u_sure') ?></p><button type='button' class='btn btn-danger' id='delete' data-action='delete'><?= lang('i_m_sure') ?></a> <button class='btn bpo-close'><?= lang('no') ?></button>" data-html="true" data-placement="left"><i class="fa fa-trash-o"></i> Xóa</a>
...
<div id="locketqua" class="form-group">
		<?php
		$_lockq = array('' => lang('All'), 'paid' => lang('paid'), 'partial' => lang('partial'), 'due' => lang('due'));
		//echo form_dropdown('dllockq', $_lockq,'', 'id="_lockq" class="form-control input-tip select" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("status") . '" required="required" style="width:100%;" ');
		?>
	</div>
<?php if ($Owner || $GP['bulk_actions']) { ?>
