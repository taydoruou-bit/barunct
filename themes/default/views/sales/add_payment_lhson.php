...
<div class="modal-dialog">
    <div class="modal-content">
        ...
        echo form_open_multipart("sales/add_phieuthukh", $attrib); ?>
        <div class="modal-body">
            <div class="row">
				<div class="col-md-12">
					<div class="form-group">					
							<?php
							echo form_input('customer_id', (isset($_POST['customer']) ? $_POST['customer'] : ""), 'id="slcustomeraddpt" data-placeholder="' . lang("select") . ' ' . lang("customer") . '" required="required" class="form-control input-tip" style="width:100%;"');
							?>
					</div>
				</div>
                <?php if ($Owner || $Admin) { ?>
                ...
                <div class="col-sm-6">
                    <div class="form-group">
                        <?= lang("Tên", "c_name"); ?>
                        <input name="c_name" type="text" id="c_name" value="<?=$ncc->name;?>" class="form-control c_name"
                               required="required"/>
                    </div>
                </div>
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <input name="c_phone" type="text" id="c_phone" value="<?=$ncc->phone;?>" class="form-control c_phone"
                               />
                    </div> 
                </div>          
                <div class="form-group">
                    <?= lang("Địa chỉ", "c_address"); ?>
                    <?php echo form_textarea('c_address', $ncc->address, 'class="form-control skip" id="c_address"'); ?>
                </div>
            </div>
        ...
        <div class="modal-footer">
            <?php echo form_submit('add_phieuthukh', lang('add_payment'), 'class="btn btn-primary"'); ?>
        </div>
    </div>
    <?php echo form_close(); ?>
</div>
...
<script type="text/javascript" charset="UTF-8">
    $(document).ready(function () {
		...
		$('#slcustomeraddpt').on('select2:select', function (e) {
        ...
        $(document).on('change', '#gift_card_no', function () {
			...
			var get_customer_id=$("#slcustomeraddpt").val();
            if (cn != '') {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        } else if (data.customer_id !== null && data.customer_id != get_customer_id) {
                            $('#gift_card_no').parent('.form-group').addClass('has-error');
