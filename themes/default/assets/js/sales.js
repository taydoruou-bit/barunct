...
if (slwarehouse = localStorage.getItem('slwarehouse')) {
 ...
 $(document).on('click', '.editkm', function () {
    ...
    $(document).on("focus", '.rquantity', function () {
		...
		console.log('id_sanpham CHANGE QTY:'+id_sanpham);
	...
	var old_price_tra;
     $(document).on("focus", '.rprice_tra', function () {
        ...
        var new_price = parseFloat($(this).val()),
        item_id = row.attr('data-item-id');
        slitems_tra[item_id].row.price = new_price;
        localStorage.setItem('slitems_tra', JSON.stringify(slitems_tra));
        loadItems_Tra();
    });
    ...
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function () {
           ...
           // unit_price = formatDecimal(unit_price+item_discount, 4);
            ...
            var newTr = $('<tr id="row_' + row_no + '" data-item-id-remove="' + item_id + '" class="row_' + item_id + '" data-item-id="' + item_id + '" data-id-khuyenmai="' + khuyenmai_main + '" data-id-sanpham="' + idsanpham + '"></tr>');
