...
$(document).ready(function() {
    ...
    $(document).on('click', '.file-caption', function(){
     ...
     $('#tmdt .iCheck-helper').click(function () {          
...
$(document).ready(function() {
    $('#daterange').daterangepicker({
        ...
        ranges: {
         ...
         'Hôm qua': [moment().subtract('days', 1).hours(0).minutes(0).seconds(0), moment().subtract('days', 1).hours(23).minutes(59).seconds(59)],
         'Tuần trước': [moment().subtract('days', 6).hours(0).minutes(0).seconds(0), moment().hours(23).minutes(59).seconds(59)],
         'Tháng trước': [moment().subtract('days', 29).hours(0).minutes(0).seconds(0), moment().hours(23).minutes(59).seconds(59)],
         'Tháng này': [moment().startOf('month').hours(0).minutes(0).seconds(0), moment().endOf('month').hours(23).minutes(59).seconds(59)],
         'Tháng trước': [moment().subtract('month', 1).startOf('month').hours(0).minutes(0).seconds(0), moment().subtract('month', 1).endOf('month').hours(23).minutes(59).seconds(59)]
     }
 }, 
 function(start, end) {
    refreshPage(start.format('YYYY-MM-DD')+' 00:00', end.format('YYYY-MM-DD')+' 23:59');
});
});
...
$(document).ready(function() {
    $(document).on('click', '.row_status', function(e) {
        ...
        if (row.hasClass('invoice_link')) {
            ...
            $('#myModal').modal('show');
        }else if (row.hasClass('khuyenmai_link')) {
            $('#myModal').modal({remote: site.base_url + 'khuyenmai/update_status/' + id});
            $('#myModal').modal('show');
...
function is_numeric(mixed_var) {
	...
	// if(!isNaN(mixed_var)&&parseFloat(mixed_var)>0){		
		// return true;
	// }	
	// return false;
}
...
function pqLhsonFormat(x) {
    if (x != null) {
        ...
        for (index = 0; index < pqc.length; ++index) {
            ...
            var v = pq.split("__");
			if(v[2]){
				d += v[0]+' ('+formatQuantity2(v[1])+' * '+formatNumber(v[2], site.settings.qty_decimals)+')<br>';
			}else{
				d += v[0]+' ('+formatQuantity2(v[1])+')<br>';
			}
        }
...
function baseToUnitQtyLhson(qty) {
	 return qty;
...
$(document).ready(function() {
    ...
    $('body').on('click', '.history_product_link td:not(:first-child, :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'products/viewapi/' + $(this).parent('.history_product_link').attr('id')});
        $('#myModal').modal('show');
    });

    $('body').on('click', '.history_order_link td:not(:first-child, :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'sales/viewapi/' + $(this).parent('.history_order_link').attr('id')});
        $('#myModal').modal('show');
    });
    ...
    $('body').on('click', '.purchase_link td:not(:first-child, :nth-child(5), :nth-last-child(2), :last-child)', function() {
        ...
        // $('#myModal').modal('show');
		console.log($(this).find(".row_status").length);		
		if($(this).find(".row_status").length>0){
			$('#myModal').modal({remote: site.base_url + 'purchases/update_status/' + $(this).parent('.purchase_link').attr('id')});
            $('#myModal').modal('show');
		}else{
			$('#myModal').modal({remote: site.base_url + 'purchases/modal_view/' + $(this).parent('.purchase_link').attr('id')});
			$('#myModal').modal('show');
    ...
    $('body').on('click', '.doncu_link td', function() {
         $('#myModal').modal('hide');   
         $('#myModal2').modal({remote: site.base_url + 'customers/viewdoncu/' + $(this).closest('tr').attr('id')});
         $('#myModal2').modal('show');
    });
    ...
    $('body').on('click', '.invoice_link td:not(:first-child, :nth-child(6), :nth-last-child(2), :last-child)', function() {
        console.log($(this).find(".row_status").length);		
		if($(this).find(".row_status").length>0){
			$('#myModal').modal({remote: site.base_url + 'sales/update_status/' + $(this).parent('.invoice_link').attr('id')});
            $('#myModal').modal('show');
		}else{
	...
	$('body').on('click', '.payment_lhson td:not(:last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'sales/payment_note/' + $(this).closest('tr').attr('id')});
        $('#myModal').modal('show');
    });
	...
	$('body').on('click', '.khuyenmai_link td:not(:first-child, :nth-last-child(3), :nth-last-child(2), :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'khuyenmai/modal_view/' + $(this).parent('.khuyenmai_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'quotes/view/' + $(this).parent('.quote_link').attr('id');
    });
    ...
    $('body').on('click', '.delivery_link td:not(:first-child, :nth-last-child(2), :nth-last-child(3), :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'sales/printgiao/' + $(this).parent('.delivery_link').attr('id')});
        $('#myModal').modal('show');
    });
    ...
    $('body').on('click', '.supplier_link td:not(:first-child)', function() {
     ...
     $('body').on('click', '.osoquy_link td:not(:first-child,:nth-last-child(1), :last-child)', function() {
        //alert(encodeURIComponent($(this).closest('tr').attr('code')));
        $('#myModal').modal({remote: site.base_url + 'the/modal_view/' + encodeURIComponent($(this).closest('tr').attr('code'))});
        $('#myModal').modal('show');  
    });
    $('body').on('click', '.soquy td:not(:first-child,:nth-last-child(2),:nth-last-child(1), :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'purchases/modal_view/' + $(this).closest('tr').attr('purchaseid')});
        $('#myModal').modal('show');  
    });
    
     $('body').on('click', '.purchase_linkdel td:not(:first-child,:nth-last-child(2),:nth-last-child(1), :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'purchases/modal_view/' + $(this).closest('tr').attr('purchaseid')});
        $('#myModal').modal('show');  
    });
    ...
    $('body').on('click', '.adjustment_link2', function() {
     ...
     $('body').on('click', '.history_full_link td', function() {
        $('#modal_history_full_id').val($(this).parent('.history_full_link').attr('id'));
        $('#modal_history_full_loai').val($(this).parent('.history_full_link').attr('theloai'));

        $data = {
            'transaction_id': $(this).parent('.history_full_link').attr('id'),
            'loai': $(this).parent('.history_full_link').attr('theloai'),
            'ADD': $(this).parent('.history_full_link').attr('ADD'),
        };
        $.ajax({
            url : site.base_url +"Apiapp/cms_view_history_full",
            type : "post",  
            data:$data,          
            success : function (result)
            {   
                if (result!='') {
                    $('#modal_history_full').html(result);
                    $("#myHistoryFull").modal('show');
