...
if (site.settings.chiec_khau != 0) {
	if (pchiec_khauv2 = localStorage.getItem('pchiec_khauv2')) {
		$('#pchiec_khauv2').val(pchiec_khauv2);
	}
}
        ...
        bootbox.confirm(lang.r_u_sure, function (result) {
            if (result) {
				...
				if (site.settings.chiec_khau != 0) {
					...
					if (localStorage.getItem('pchiec_khauv2')) {
						localStorage.removeItem('pchiec_khauv2');
					}
				}
...
var old_pchiec_khau;
$('#pchiec_khau').focus(function () {
    old_pchiec_khau = $(this).val();
}).change(function () {
	
    if (is_valid_discount($(this).val())) {
        localStorage.removeItem('pchiec_khau');
        localStorage.setItem('pchiec_khau', $(this).val());
        loadItems();
        return;
    } else {
        $(this).val(old_pchiec_khau);
        bootbox.alert(lang.unexpected_value);
        return;
    }
     ...
     * Edit Row Modal Hanlder
     ...
     $(document).on('click', '.edit', function () {
        ...
        if (site.settings.tax1) {
			...
			var item_chieckhau = 0, dsck = chiec_khau ? chiec_khau : '0';
            if (dsck.indexOf("%") !== -1) {
                var pds = dsck.split("%");
                if (!isNaN(pds[0])) {
                    item_chieckhau = parseFloat(((real_unit_cost-item_discount) * parseFloat(pds[0])) / 100);
                } else {
                    item_chieckhau = parseFloat(dsck);
                }
            } else {
                item_chieckhau = parseFloat(dsck);
            }
			
            net_cost -= item_discount - item_chieckhau;
    ...
    $(document).on('change', '#pcost, #ptax, #pdiscount, #pchiec_khauv2', function () {
        var row = $('#' + $('#row_id').val());
        var item_id = row.attr('data-item-id');
        ...
        var pr_tax = $('#ptax').val(), item_tax_method = item.row.tax_rate;
		...
		var ck = $('#pchiec_khauv2').val() ? $('#pchiec_khauv2').val() : '0';
		var chiec_khau=0;
        if (ck.indexOf("%") !== -1) {
            var pds_ck = ck.split("%");
            if (!isNaN(pds_ck[0])) {
                chiec_khau = parseFloat(((unit_cost) * parseFloat(pds_ck[0])) / 100);
            } else {
                chiec_khau = parseFloat(ck);
            }
        } else {
            chiec_khau = parseFloat(ck);
        }
		unit_cost=eval(unit_cost+"-"+chiec_khau);
     ...
     * Edit Row Method
     ...
     $(document).on('click', '#editItem', function () {
		...
		poitems[item_id].row.chiec_khau = $('#pchiec_khauv2').val() ? $('#pchiec_khauv2').val() : '0',
...
function loadItems() {
    ...
    if (localStorage.getItem('poitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_cost = item.row.cost, item_oqty = item.row.oqty, item_qty = item.row.qty, item_bqty = item.row.quantity_balance, item_expiry = item.row.expiry, item_tax_method = item.row.tax_rate, item_ds = item.row.discount, item_discount = 0,i_chiec_khauv2=item.row.chiec_khau;item_chieckhau=0, item_option = item.row.option, item_code = item.row.code, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
            ...
            var supplier = localStorage.getItem('posupplier'), belong = false;
                ...
                if(item.row.fup != 1 && product_unit != item.row.base_unit) {
                    $.each(item.units, function(){
                        if (this.id == product_unit) {
							...
							unit_cost = formatDecimal((parseFloat(item.row.base_unit_price)*(unitToBaseQty(1, this))), 4);
				...
				product_discount_row = parseFloat(item_discount * item_qty);
				
						
				var ds_ck = i_chiec_khauv2 ? i_chiec_khauv2 : '0';

                if (ds_ck.indexOf("%") !== -1) {
                    var pds = ds_ck.split("%");
                    if (!isNaN(pds[0])) {
                        item_chieckhau= formatDecimal((parseFloat(((unit_cost-item_discount) * parseFloat(pds[0])) / 100)), 4);
                    } else {
                        item_chieckhau = formatDecimal(ds_ck);
                    }
                } else {
                     item_chieckhau = formatDecimal(ds_ck);
                }
				console.log(item_chieckhau);
				product_chieckhau += parseFloat(item_chieckhau * item_qty);
				
				product_chieckhau_row = parseFloat(item_chieckhau * item_qty);
                ...
                unit_cost = formatDecimal(unit_cost, 4);
                var sel_opt = '';
                $.each(item.options, function () {
			...
			if (site.settings.chiec_khau == 1) {
                tr_html += '<td class="text-right"><input class="form-control input-sm rchiec_khau" name="product_chieckhau[]" type="hidden" id="discountck_' + row_no + '" value="' + i_chiec_khauv2 + '"><span class="text-right schieckhau text-danger" id="schieckhau_' + row_no + '">' + formatMoney(0 - (product_chieckhau_row)) + '</span></td>';
            }
            if (site.settings.tax1 == 1) {
            ...
            tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_cost) + parseFloat(pr_tax_val)) * parseFloat(item_qty))-product_discount_row-product_chieckhau_row) + '</span></td>';
            tr_html += '<td class="text-center"><i class="fa fa-times tip podel" id="' + row_no + '" title="Remove" style="cursor:pointer;"></i></td>';
            newTr.html(tr_html);
            newTr.prependTo("#poTable");
            total += formatDecimal(((parseFloat(item_cost) + parseFloat(pr_tax_val)) * parseFloat(item_qty))-product_discount_row-product_chieckhau_row, 4);
        ...
        if (site.settings.product_discount == 1) {
		 ...
		 if (site.settings.chiec_khau == 1) {
            tfoot += '<th class="text-right">'+formatMoney(product_chieckhau)+'</th>';
        }
        if (site.settings.tax1 == 1) {
		...
		//chiec khau ap dung moi lhson 
		if (site.settings.chiec_khau != 0) {
			if (pchiec_khau = localStorage.getItem('pchiec_khau')) {
				var ds_ck = pchiec_khau;
				if (ds_ck.indexOf("%") !== -1) {
					var pds_ck = ds_ck.split("%");
					if (!isNaN(pds_ck[0])) {
						tong_chiec_khau = formatDecimal(((order_discount * parseFloat(pds_ck[0])) / 100), 4);
					} else {
						tong_chiec_khau = formatDecimal(ds_ck);
					}
				} else {
					tong_chiec_khau = formatDecimal(ds_ck);
				}
        ...
        total_discount = parseFloat(order_discount + product_discount + tong_chiec_khau);
        // Totals calculations after item addition
        var gtotal = ((total + invoice_tax) - order_discount - tong_chiec_khau) + shipping;
        $('#tong_chiec_khau').text(formatMoney(tong_chiec_khau));
 ...
 function add_purchase_item(item) {
    if (count == 1) {
        ...
        if ($('#posupplier').val()) {
            ...
            bootbox.alert('Vui lòng chọn nhà cung cấp');
            item = null;
            return;
        }
    }
