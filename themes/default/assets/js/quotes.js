...
$(document).on('click', '#editItem', function () {
    ...
    // ✅ THÊM ĐOẠN NÀY - LƯU CUSTOM FIELDS TỪ HÀNG HIỆN TẠI
    var current_custom_fields = {};
    row.find('.custom-field-input').each(function() {
        var fieldName = $(this).attr('name').replace('[]', '');
        var fieldValue = $(this).val();
        current_custom_fields[fieldName] = fieldValue;
    });
    
    // Giữ lại custom fields hiện tại
    if (Object.keys(current_custom_fields).length > 0) {
        quitems[item_id].custom_fields = current_custom_fields;
    }
    
    // ✅ THÊM ĐOẠN NÀY - LƯU NOTES TỪ HÀNG HIỆN TẠI
    var current_notes = row.find('.rnotes').val();
    if (current_notes !== undefined) {
        quitems[item_id].notes = current_notes;
    }
...
function loadItems() {
    ...
    if (localStorage.getItem('quitems')) {
        ...
        $.each(quitems, function (key, value) {  // ← Duyệt quitems thay vì sortedItems
            ...
            // ✅ TÌM ĐOẠN NÀY TRONG loadItems() VÀ KIỂM TRA
if (customColumns && customColumns.length > 0) {
    customColumns.forEach(function (columnName) {
        ...
        // ✅ TÍNH TOÁN col ĐÚNG với các cột tùy chỉnh
// ✅ TÍNH TOÁN col ĐÚNG với các cột tùy chỉnh
var col = 2; // Cột "Product Name"

// Thêm số cột tùy chỉnh
if (customColumns && customColumns.length > 0) {
...
function add_invoice_item(item) {
    ...
    // TẠO ID MỚI BẰNG TIMESTAMP thay vì dùng item.id
    var item_id = (new Date()).getTime();  // ← SỬA DÒNG NÀY

    // XÓA TOÀN BỘ BLOCK IF (quitems[item_id])
quitems[item_id] = item;
if (!quitems[item_id].custom_fields) {
    quitems[item_id].custom_fields = {};
}
if (!quitems[item_id].notes) {  // ← THÊM DÒNG NÀY
    quitems[item_id].notes = '';  // ← THÊM DÒNG NÀY
}
...
/* --------------------------
 * Edit Row Notes Method
 -------------------------- */
var old_notes;
$(document).on("focus", '.rnotes', function () {
    old_notes = $(this).val();
}).on("change", '.rnotes', function () {
    var row = $(this).closest('tr');
    var new_notes = $(this).val();
    var item_id = row.attr('data-item-id');
    quitems[item_id].notes = new_notes;  // ← THÊM DÒNG NÀY
    localStorage.setItem('quitems', JSON.stringify(quitems));
});