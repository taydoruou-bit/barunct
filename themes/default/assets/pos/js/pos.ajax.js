...
$(document).on('blur', '#sale_note', function () {
 ...
 if (localStorage.getItem('dppos')) {
    localStorage.removeItem('dppos');
}
if (localStorage.getItem('canmppos')) {
    localStorage.removeItem('canmppos');
}
if (localStorage.getItem('vienmppos')) {
    localStorage.removeItem('vienmppos');
}
if (localStorage.getItem('loanmppos')) {
    localStorage.removeItem('loanmppos');
}
if (localStorage.getItem('addmppos')) {
    localStorage.removeItem('addmppos');
}
if (localStorage.getItem('tmmppos')) {
    localStorage.removeItem('tmmppos');
}
if (localStorage.getItem('canmtpos')) {
    localStorage.removeItem('canmtpos');
}
 if (localStorage.getItem('vienmtpos')) {
    localStorage.removeItem('vienmtpos');
}
if (localStorage.getItem('loanmtpos')) {
    localStorage.removeItem('loanmtpos');
}
if (localStorage.getItem('addmtpos')) {
    localStorage.removeItem('addmtpos');
}
if (localStorage.getItem('tmmtpos')) {
    localStorage.removeItem('tmmtpos');
}
...
$('#posdiscount').focus(function () {
    ...
    $(document).on('click', '#editItem', function () {
        ...
        //check trùng option, remove option hiện có, sl thay bằng sl hiện tại
        $option_get=$('#poption').val() ? $('#poption').val() : '';
        $check_trung=0;
        $.each(positems, function () {
            var obj = this;           
            if (obj.row.option==$option_get&&obj.row.option!=positems[item_id].row.option)
            {
                $check_trung++;
            }
        });
        ...
        if ($check_trung>0) {
            $.each(positems, function () {
                var obj = this; 
                new_qty = 0;
                if (obj.row.option==$option_get)
                {
                    //remove obj item
                    delete positems[obj.item_id];                
                    $("#row_"+obj.item_id).remove();
                }
            });
        }
    ...
    $(document).on("focus", '.rquantity', function () {
        ...
        if (!is_numeric($(this).val()) || parseFloat($(this).val()) < 0) {
            $(this).val(old_row_qty);
            bootbox.alert('Vui lòng nhập số');
            return;
        }
                        ...
                        jQuery.each( list_sub_pr, function( i, val ) {
                            ...
                            $.ajax({
                                ...
                                success: function (ui) {
                                    ...
                                    console.log('id_sanpham CHANGE QTY DONE:');
                                }
                            });
...
//localStorage.clear();
function loadItems(item_id_add=0) {
    ...
    if (localStorage.getItem('positems')) {
        ...
        if (pos_settings.remote_printing != 1) {
            ...
            var pos_customer = 'Khách: '+$('#select2-chosen-1').text()+ "\n";
            var hr = ''+$('#reference_note').val()+ "\n";
            var user = 'Nhân viên: '+username+ "\n";
            ...
            var pos_head2 = '</h4><p class="text-left">Khách: '+$('#s2id_poscustomer').text()+'<br>'+$('#reference_note').val()+'<br>Nhân viên: '+username+'<br>T: '+date(site.dateFormats.php_ldate, time)+'</p></span>';
        ...
        $.each(sortedItems, function () {
           ...
           // unit_price = formatDecimal((unit_price+item_discount), 4);
            ...
            tr_html += '<input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="p_gia_set_giasi" name="p_gia_set_giasi[]" type="hidden" value="' + p_gia_set_giasi + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(parseFloat(item_price) + parseFloat(pr_tax_val)) + '</span></td>';
            tr_html += '<td><input class="form-control input-sm kb-pad text-center rquantity" tabindex="'+((site.settings.set_focus == 1) ? an : (an+1))+'" name="quantity[]" type="text" value="' + parseFloat(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"><input name="product_unit[]" type="hidden" class="runit" value="' + product_unit + '"><input name="product_base_quantity[]" type="hidden" class="rbase_quantity" value="' + base_quantity + '"><input name="data_id_khuyenmai[]" type="hidden" class="data_id_khuyenmai" value="' + khuyenmai_main + '"></td>';
            tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty))-formatDecimal(item_discount * item_qty)) + '</span></td>';
            ...
            total += formatDecimal(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty))-formatDecimal(item_discount * item_qty), 4);
        ...
        if (pos_settings.remote_printing != 1) {
			...
			var cus_id_get=parseInt(localStorage.getItem('poscustomer')); 
			var tong_no_cu=localStorage.getItem('tong_no_kh'+cus_id_get);
			
			bill_totals += '<tr class="bold"><td>Tổng nợ củ:</td><td style="text-align:right;">' + (tong_no_cu) + '</td></tr>';
 ...
 * @returns {Boolean}
 ...
 function add_invoice_item(item,checkedit=null) {
    ...
    //var item_id = site.settings.item_addition == 1 ? item.item_id : item.id;
    var item_id = item.id;
    var id_sanpham = item.row.id;

    var new_qty =1;
    var check_countinue=0;
    //lap qua tat ca san pham dang co
    $.each(positems, function () {
        var obj = this;         
        if (obj.row.id==item.row.id) {
            if (obj.row.option==item.row.option)
            {          
                
                if (obj.row.khuyenmai_main>0&&checkedit!=null) {
                    
                    //tien hanh xoa km cu             
                    delete positems[obj.item_id];
               
                }else{
                    //them vao sp co khuyen mai
                    if (obj.row.khuyenmai_main==""||obj.row.khuyenmai_main==0||obj.row.khuyenmai_main==undefined) {
                        item_id=obj.item_id;
                        check_countinue++;
                        new_qty = parseFloat(positems[obj.item_id].row.qty) + 1; 
    ...
    if (check_countinue>0) {
        if (item.row.khuyenmai_main>0&&checkedit!=null) {
            item_id=item.id;
            ...
            if(positems[item_id].row.unit != positems[item_id].row.base_unit) {
                $.each(positems[item_id].units, function(){
                    if (this.id == positems[item_id].row.unit) {
                        positems[item_id].row.base_quantity = baseToUnitQty(new_qty, this);
                    }
                });
            }
            localStorage.setItem('positems', JSON.stringify(positems));
    ...
    if(khuyenmai_main&&checkedit==null){
        ...
        // kiem tra xem san pham co phai la san pham khuyen mai khong
        if(check!==-1){
            console.log('LA SP Khuyenmai');
                            
            //kiem tra xem so luong >= main_quantity
            if(positems[item_id]!=undefined&&(positems[item_id].row.qty>=khuyenmai_main[id_sanpham].main_quantity)){
                ...
                jQuery.each( list_sub_pr, function( i, val ) {
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        data: {
                            term:sub_product_id,quantity:sub_quantity,price:giakhuyenmai,khuyenmai_main:id_sanpham
                        },
                        success: function (ui) {
                            ui.row.code='[KHUYẾN MÃI]';
                            ui.row.khuyenmai_main=id_sanpham;
