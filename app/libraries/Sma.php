...
class Sma
{
    ...
    public function checkPermissions($action = null, $js = null, $module = null)
    {
        if (!$this->actionPermissions($action, $module)) {
            ...
            if ($js) {
                $this->sma->send_json(array('error' => 0, 'msg' =>lang("access_denied")));    
	...
	public function checkPermissionsReturn($action = null, $js = null, $module = null)
    {
        if (!$this->actionPermissions($action, $module)) {
            return false;
        }
    ...
    {
        $error_reporting = error_reporting();
        error_reporting($error_reporting & ~E_DEPRECATED & ~E_USER_DEPRECATED & ~E_WARNING & ~E_NOTICE & ~E_STRICT);
        ...
        try {
            ...
            show_error('Không thể tạo file PDF. Vui lòng thử lại hoặc liên hệ kỹ thuật.', 500);
        } finally {
            error_reporting($error_reporting);
        }
    }

    public function generate_pdf_to_image($content, $name = 'download.jpg', $output_type = null, $footer = null, $margin_bottom = null, $header = null, $margin_top = null, $orientation = 'P')
    {
        if (!$output_type) {
            $output_type = 'D';
        }
        ...
        $this->load->library('pdf');
        $pdf = new mPDF('utf-8', 'A4-' . $orientation, '13', '', 5, 5, $margin_top, $margin_bottom, 9, 9);
        $pdf->debug = false;
        $pdf->autoScriptToLang = true;
        $pdf->autoLangToFont = true;
        // if you need to add protection to pdf files, please uncomment the line below or modify as you need.
        ...
        $file_content = $pdf->Output('', 'S');
        write_file('assets/print/' . $name, $file_content);
        return 'assets/print/' . $name;
    ...
    public function update_award_pointsbk($total, $customer, $user, $scope = null)
    {
    ...
    public function update_award_points($total, $customer=0, $user=0, $scope = null,$total_gram=0)
    {
        if ((int)$user==0) {
            $user=1;
        }
        ...
        if (!empty($this->Settings->each_gram) && $total_gram >0) {

            $company = $this->site->getCompanyByID($customer);

            $points = (double)(($total_gram / $this->Settings->each_gram) * $this->Settings->ca_gram);
    ...
    public function paid_optsbk($paid_by = null, $purchase = false, $empty_opt = false)
    {
        $opts = '';
        if ($empty_opt) {
	...
	public function paid_opts_lhsonbk($paid_by = null, $purchase = false, $empty_opt = false)
    {
        $opts = '';
        if ($empty_opt) {
            $opts .= '<option value="">'.lang('select').'</option>';
        ...
        $opts .= '
        <option value="cash"'.($paid_by && $paid_by == 'cash' ? ' selected="selected"' : '').'>'.lang("cash").'</option>
        <option value="chuyenkhoan"'.($paid_by && $paid_by == 'chuyenkhoan' ? ' selected="selected"' : '').'>'.lang("Chuyển khoản").'</option>
        <option value="CC"'.($paid_by && $paid_by == 'CC' ? ' selected="selected"' : '').'>'.lang("CC").'</option>
        <option value="Cheque"'.($paid_by && $paid_by == 'Cheque' ? ' selected="selected"' : '').'>'.lang("cheque").'</option>
        <option value="other"'.($paid_by && $paid_by == 'other' ? ' selected="selected"' : '').'>'.lang("other").'</option>';
    ...
    public function paid_opts($paid_by = null, $purchase = false, $empty_opt = false)
    {
        ...
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                if (!$purchase) {
                    $opts .= '<option value="'.$row->code.'"'.($paid_by && $paid_by == $row->code ? ' selected="selected"' : '').'>'.$row->name.'</option>';
                }else{
                    if ($row->code!='deposit') {
                        $opts .= '<option value="'.$row->code.'"'.($paid_by && $paid_by == $row->code ? ' selected="selected"' : '').'>'.$row->name.'</option>';
                    }
        ...
        if (!$purchase)
        {
            $opts .= '<option value="gift_card_point"'.($paid_by && $paid_by == 'gift_card_point' ? ' selected="selected"' : '').'>Điểm tích lũy</option>';
            $opts .= '<option value="deposit"'.($paid_by && $paid_by == 'deposit' ? ' selected="selected"' : '').'>Tiền cọc</option>';
        }

        return $opts;
    }
    public function paid_opts_lhson($paid_by = null, $purchase = false, $empty_opt = false)
    {
        $opts = '';
        if ($empty_opt) {
            $opts .= '<option value="">'.lang('select').'</option>';
