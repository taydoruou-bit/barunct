...
class Phuthu_model extends CI_Model
{
    ...
    {
        $q = $this->db->get_where('phuthu', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function addPurchase($data, $items)
    {
        ...
        if ($this->db->insert('phuthu', $data)) {
            ...
            if ($data['status'] == 'returned') {
                $this->db->update('phuthu', array('return_purchase_ref' => $data['return_purchase_ref'], 'surcharge' => $data['surcharge'],'return_purchase_total' => $data['grand_total'], 'return_id' => $purchase_id), array('id' => $data['purchase_id']));
            }
    ...
    public function updatePurchase($id, $data, $items = array())
    {
        ...
        if ($this->db->update('phuthu', $data, array('id' => $id)) && $this->db->delete('purchase_items', array('purchase_id' => $id))) {
            $purchase_id = $id;
            foreach ($items as $item) {
                $item['purchase_id'] = $id;
                $this->db->insert('purchase_items', $item);
    ...
    public function updateStatus($id, $status, $note)
    {
        ...
        if ($this->db->update('phuthu', array('status' => $status, 'note' => $note), array('id' => $id))) {
            foreach ($items as $item) {
    ...
    public function deletePurchase($id)
    {
        ...
        if ($this->db->delete('purchase_items', array('purchase_id' => $id)) && $this->db->delete('phuthu', array('id' => $id))) {
    ...
    {
        $q = $this->db->get_where('return_phuthu', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function returnPurchase($data = array(), $items = array())
    {
        ...
        if ($this->db->insert('return_phuthu', $data)) {
    ...
    public function calculatePurchaseTotals($id, $return_id, $surcharge)
    {
        ...
        if (!empty($items)) {
            ...
            if ($this->db->update('phuthu', $data, array('id' => $id))) {
                return true;
            }
        } else {
            $this->db->delete('phuthu', array('id' => $id));
        }
        return FALSE;
    }
