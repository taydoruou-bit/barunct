...
class Pos_model extends CI_Model
{
    ...
    public function fetch_products($category_id, $limit, $start, $subcategory_id = NULL, $brand_id = NULL)
    {
        ...
        if ($brand_id) {
            ...
            //get all subcategory
            if ((int)$subcategory_id==0)
            {
                //lay danh muc cha
                $obj_parent=$this->site->getSubCategoriesV2($category_id);
                if (!empty($obj_parent)) 
                {
                    foreach ($obj_parent as $cat) 
                    {
                        
                        $obj_parent2=$this->site->getSubCategoriesV2($cat->id);
                        if (!empty($obj_parent2)) 
                        {
                            foreach ($obj_parent2 as $cat2) 
                            {
                                $add_where_cat[]=$cat2->id;
                            }
                        }else{
                            $add_where_cat[]=$cat->id;
                        }
                    }                                                                  
                }else{
                    $add_where_cat[]=$category_id;
                }                           
            }else{
                $add_where_cat[]=$subcategory_id;
            }
        }
        if ($subcategory_id) {
            
            $obj_parent=$this->site->getSubCategoriesV2($subcategory_id);
            if (!empty($obj_parent)) 
            {
                foreach ($obj_parent as $cat) 
                {
        ...
        // echo var_dump($add_where_cat);
        // die();

        if (!empty($add_where_cat)) {
            $this->db->where_in('category_id', $add_where_cat);
            $this->db->or_where_in('subcategory_id', $add_where_cat);
        }
    ...
    public function fetch_products_api($category_id=null, $limit=null, $offset=null, $subcategory_id = NULL, $brand_id = NULL,$term='',$orderby='')
    {
        ...
        if ($term!='')
        {
            $this->db->where("({$this->db->dbprefix('products')}.name LIKE '%" . $term . "%' OR {$this->db->dbprefix('products')}.code LIKE '%" . $term . "%')");
        }
        if ($orderby!='') {
            if ($orderby=='name') {
                $this->db->order_by("name", "asc");        
            }elseif ($orderby=='moinhat') {
                $this->db->order_by("id", "desc");        
            }else {
                $this->db->join('sale_items', 'sale_items.product_id=products.id', 'left');
                $this->db->group_by('products.id');
                //join to sale_items
                $this->db->order_by("sum(scodeweb_sale_items.quantity)", "desc");    
            }
            
        }
        
        $query = $this->db->select('products.*')->get("products");
    ...
    {
        $this->db->select('products.*,warehouses_products.product_id, warehouses_products.quantity, categories.id as category_id, categories.name as category_name')
    ...
    public function addSale($data = array(), $items = array(), $payments = array(), $sid = NULL,$giaohang=null)
    {
        ...
        if ($this->db->insert('sales', $data)) {
           ...
           /*luu lich su tao hoa don lhson code 05/09/2021*/
            ...
            return array('sale_id' => $sale_id,'delivery_id' => $delivery_id, 'message' => $msg);
    ...
    public function getAllInvoiceHistoryItems($sale_id)
    {
        if ($this->pos_settings->item_order == 0) {
            $this->db->select('sale_items_history.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, product_variants.name as variant, products.details as details,product_variants.name as variant,products.baohanh as baohanh')
            ->join('products', 'products.id=sale_items_history.product_id', 'left')
            ->join('tax_rates', 'tax_rates.id=sale_items_history.tax_rate_id', 'left')
            ->join('product_variants', 'product_variants.id=sale_items_history.option_id', 'left')
            ->group_by('sale_items_history.id')
            ->order_by('id', 'asc');
        } elseif ($this->pos_settings->item_order == 1) {
            $this->db->select('sale_items_history.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, product_variants.name as variant, categories.id as category_id, categories.name as category_name, products.details as details')
            ->join('tax_rates', 'tax_rates.id=sale_items_history.tax_rate_id', 'left')
            ->join('product_variants', 'product_variants.id=sale_items_history.option_id', 'left')
            ->join('products', 'products.id=sale_items_history.product_id', 'left')
            ->join('categories', 'categories.id=products.category_id', 'left')
        ...
        $q = $this->db->get_where('sale_items_history', array('sale_id' => $sale_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
        if ($this->pos_settings->item_order == 0) {
            $this->db->select('sale_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, product_variants.name as variant, products.details as details,product_variants.name as variant,products.baohanh as baohanh')
    ...
    public function getInvoiceHistoryByID($id)
    {

        $q = $this->db->get_where('sales_history', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function fetch_bills($limit=null, $start=null,$warehouse_id=null)
    {
        if (!$this->Owner && !$this->Admin) {
            $this->db->where('created_by', $this->session->userdata('user_id'));
        }
        if (($this->Owner || $this->Admin) && $warehouse_id>0) {
     ...
     public function fetch_bills_api($limit=0, $start=0,$warehouse_id=null)
    {
        if (!$this->Owner && !$this->Admin) {
            $this->db->where('created_by', $this->session->userdata('user_id'));
        }
    ...
    public function suspendSaleApi($data = array(), $items = array(), $did = NULL)
    {
        $sData = array(
    ...
    public function getInvoiceHistoryPayments($sale_id)
    {
        $q = $this->db->get_where("payments_history", array('sale_id' => $sale_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
