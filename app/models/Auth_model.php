...
class Auth_model extends CI_Model
{
	...
	function showPassReset($password)
	{
		$salt = $this->store_salt ? $this->salt() : FALSE;
        echo $this->hash_password($password, $salt);
	}
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $query = $this->db->select($this->identity_column . ', username,first_name,last_name, email, id, password, active, last_login, last_ip_address, avatar, gender, group_id, warehouse_id, biller_id, company_id, view_right, edit_right, allow_discount, show_cost, show_price')
	...
	function scodeweb_username(){
		$q = $this->db->get_where("scodeweb_settings", array('setting_id' => 1), 1);  
	...
	public function readPostUrl($json) {

		return 'OK';    
	}
    public function updatePriceProductLhson($id=0,$price=0,$name='')
    {
        if ($this->db->update('products', array('price' => $price,'name' => $name), array('id_woo' => $id))) {
            return TRUE;
        }
