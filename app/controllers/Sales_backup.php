...
class Sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
				->select("id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, biller, concat(customer,'<br/>',(select phone from scodeweb_companies where id=customer_id)) as customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, attachment, return_id")
				->from('sales')
    ...
    public function edit($id = null)
    {
        ...
        if ($inv->sale_status == 'returned' || $inv->return_id || $inv->return_sale_ref) {
            $this->session->set_flashdata('error', lang('Đơn hàng có thu hồi sản phẩm không thể cập nhật'));
	...
	public function printsalelhson($id = null)    {      
		...
		foreach ($rows as $row):        
			...
			$_prod_detail.=$row->details ? '<br>' . $row->details : '';        
			$_strgia=$this->sma->formatMoney($row->unit_price);
			if ($Settings->product_discount && $inv->product_discount != 0) {
				$_strgia.=($row->discount != 0 ? '<small>(' . $row->discount . ')</small> ' : '') . $this->sma->formatMoney($row->item_discount);        
			}    
		...
		if ($return_rows) {        
			...
			$_rt_prod=$row->product_code.' - '.$row->product_name . ($row->variant ? ' (' . $row->variant . ')' : '');
			$_rt_prod.=$row->details ? '<br>' . $row->details : '';        
			$_rt_strgia=$this->sma->formatMoney($row->unit_price);        
			if ($Settings->product_discount && $inv->product_discount != 0) {    
				$_rt_strgia.=($row->discount != 0 ? '<small>(' . $row->discount . ')</small> ' : '') . $this->sma->formatMoney($row->item_discount);        
			}            
		...
		$parse_data = array('Ma_Don_Hang' => $inv->reference_no,'Khach_Hang' => $customer->name,'Cong_ty' => $customer->company,'site_link' => base_url(),'site_name' => $this->Settings->site_name,'logo' => '<img src="' . base_url() . 'assets/uploads/logos/' . $biller->logo . '" alt="' . ($biller->company != '-' ? $biller->company : $biller->name) . '"/>','Ten_Cua_Hang' => $warehouse->name,'Dia_Chi_Cua_Hang' =>$_dc_cuahang,'SDT_Cua_Hang' =>$warehouse->phone,'Dia_chi_kh' => $customer->address,'Dien_thoai_kh' => $customer->phone,'Ngay_Xuat' => $this->sma->hrld($inv->date),'Tong_Thanh_Toan' =>$_tongthanhtoan,'Nhan_Vien_Thu_Ngan' =>$created_by->first_name . ' ' . $created_by->last_name,'Phu_Phi' => $this->sma->formatMoney($inv->shipping),'Ghi_Chu' =>$this->sma->decode_html($inv->note),'No_cu' =>$_tong_no_cu,'Chua_Thanh_Toan' => $_chuathanhtoan,'Da_Thanh_Toan' => $tong_dathanhtoan,'Tong_Diem_Tich_Luy' =>$_tong_diem,'Diem_hoa_don' =>$_diem_thuong,'Giam_Gia_Tren_Hoa_Don' =>$_giamgia,'Tong_Tien_Hang' =>$_tongcong,'Tong_thu_hoi' =>$_tongthuhoi,'Bang_Hoa_Don' =>$_tablhd,            );    
		
		if (file_exists('./themes/' . $this->theme . '/views/print_templates/printsale.html')) {          
			$sale_temp = file_get_contents('themes/' . $this->theme . '/views/print_templates/printsale.html');            
		} else {             
			$sale_temp = file_get_contents('./themes/default/views/print_templates/printsale.html');          
		}        
