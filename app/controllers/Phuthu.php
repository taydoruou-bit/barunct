...
class Phuthu extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->lang->load('phuthu', $this->Settings->user_language);
        $this->load->library('form_validation');
        $this->load->model('phuthu_model');
    ...
    public function index($warehouse_id = null)
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('phuthu')));
        $meta = array('page_title' => lang('phuthu'), 'bc' => $bc);
        $this->page_construct('phuthu/index', $meta, $this->data);
    ...
    public function modal_view($purchase_id = null)
    {
        ...
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $inv = $this->phuthu_model->getPurchaseByID($purchase_id);
        if (!$this->session->userdata('view_right')) {
            $this->sma->view_rights($inv->created_by, true);
        }
        $this->data['rows'] = $this->phuthu_model->getAllPurchaseItems($purchase_id);
        $this->data['supplier'] = $this->site->getCompanyByID($inv->supplier_id);
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
        $this->data['payments'] = $this->phuthu_model->getPaymentsForPurchase($purchase_id);
        $this->data['created_by'] = $this->site->getUser($inv->created_by);
        $this->data['updated_by'] = $inv->updated_by ? $this->site->getUser($inv->updated_by) : null;
        $this->data['return_purchase'] = $inv->return_id ? $this->phuthu_model->getPurchaseByID($inv->return_id) : NULL;
        $this->data['return_rows'] = $inv->return_id ? $this->phuthu_model->getAllPurchaseItems($inv->return_id) : NULL;

        $this->load->view($this->theme . 'phuthu/modal_view', $this->data);
    ...
    public function view($purchase_id = null)
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('phuthu'), 'page' => lang('phuthu')), array('link' => '#', 'page' => lang('view')));
        $meta = array('page_title' => lang('view_purchase_details'), 'bc' => $bc);
        $this->page_construct('phuthu/view', $meta, $this->data);
...
//generate pdf and force to download
    ...
    public function pdf($purchase_id = null, $view = null, $save_bufffer = null)
    {
        ...
        $this->data['return_purchase'] = $inv->return_id ? $this->phuthu_model->getPurchaseByID($inv->return_id) : NULL;
        $this->data['return_rows'] = $inv->return_id ? $this->phuthu_model->getAllPurchaseItems($inv->return_id) : NULL;
        $name = $this->lang->line("phuthu") . "_" . str_replace('/', '_', $inv->reference_no) . ".pdf";
        $html = $this->load->view($this->theme . 'phuthu/pdf', $this->data, true);
        if (! $this->Settings->barcode_img) {
            $html = preg_replace("'\<\?xml(.*)\?\>'", '', $html);
        }
        if ($view) {
            $this->load->view($this->theme . 'phuthu/pdf', $this->data);
        } elseif ($save_bufffer) {
    ...
    public function email($purchase_id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->sma->send_email($to, $subject, $message, null, null, $attachment, $cc, $bcc)) {
            ...
            $this->db->update('phuthu', array('status' => 'ordered'), array('id' => $purchase_id));
            ...
            $this->load->view($this->theme . 'phuthu/email', $this->data);
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    $product_details = $this->phuthu_model->getProductByCode($item_code);
                    if ($item_expiry) {
                        $today = date('Y-m-d');
        ...
        if ($this->form_validation->run() == true && $this->phuthu_model->addPurchase($data, $products)) {
            ...
            if ($quote_id) {
                $this->data['quote'] = $this->phuthu_model->getQuoteByID($quote_id);
                $supplier_id = $this->data['quote']->supplier_id;
                $items = $this->phuthu_model->getAllQuoteItems($quote_id);
                krsort($items);
                ...
                foreach ($items as $item) {
                    ...
                    if ($row->type == 'combo') {
                        ...
                        foreach ($combo_items as $citem) {
                            ...
                            $crow->expiry = '';
                            $options = $this->phuthu_model->getProductOptions($crow->id);
                        ...
                        $options = $this->phuthu_model->getProductOptions($row->id);
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('purchases'), 'page' => lang('phuthu')), array('link' => '#', 'page' => lang('add_purchase')));
            $meta = array('page_title' => lang('add_purchase'), 'bc' => $bc);
            $this->page_construct('phuthu/add', $meta, $this->data);
        }
    }
    ...
    public function edit($id = null)
    {
        ...
        $inv = $this->phuthu_model->getPurchaseByID($id);
        ...
        if ($this->form_validation->run() == true && $this->phuthu_model->updatePurchase($id, $data, $products)) {
            $this->session->set_userdata('remove_pols', 1);
            $this->session->set_flashdata('message', $this->lang->line("purchase_added"));
            redirect('phuthu');
        } else {
            ...
            $inv_items = $this->phuthu_model->getAllPurchaseItems($id);
            krsort($inv_items);
            $c = rand(100000, 9999999);
            ...
            $this->data['id'] = $id;
            $this->data['suppliers'] = $this->site->getAllCompanies('supplier');
            $this->data['purchase'] = $this->phuthu_model->getPurchaseByID($id);
            $this->data['categories'] = $this->site->getAllCategories();
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->load->helper('string');
            ...
            $this->session->set_userdata('remove_pols', 1);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('phuthu'), 'page' => lang('phuthu')), array('link' => '#', 'page' => lang('edit_purchase')));
            $meta = array('page_title' => lang('edit_purchase'), 'bc' => $bc);
            $this->page_construct('phuthu/edit', $meta, $this->data);
        }
    }
    ...
    public function purchase_by_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                if (!$this->upload->do_upload()) {
                    $error = $this->upload->display_errors();
                    $this->session->set_flashdata('error', $error);
                    redirect("phuthu/purchase_by_csv");
                }
                ...
                foreach ($final as $csv_pr) {
                    ...
                    if (isset($csv_pr['code']) && isset($csv_pr['net_unit_cost']) && isset($csv_pr['quantity'])) {
                        ...
                        if ($product_details = $this->phuthu_model->getProductByCode($csv_pr['code'])) {

                            if ($csv_pr['variant']) {
                                $item_option = $this->phuthu_model->getProductVariantByName($csv_pr['variant'], $product_details->id);
                                if (!$item_option) {
                            ...
                            if (isset($item_tax_rate) && $item_tax_rate != 0) {
                                ...
                                if ($tax_details = $this->phuthu_model->getTaxRateByName($item_tax_rate)) {
        ...
        if ($this->form_validation->run() == true && $this->phuthu_model->addPurchase($data, $products)) {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('phuthu'), 'page' => lang('phuthu')), array('link' => '#', 'page' => lang('add_purchase_by_csv')));
            $meta = array('page_title' => lang('add_purchase_by_csv'), 'bc' => $bc);
            $this->page_construct('phuthu/purchase_by_csv', $meta, $this->data);
    ...
    public function delete($id = null)
    {
        ...
        if ($this->phuthu_model->deletePurchase($id)) {
    ...
    public function suggestions()
    {
        ...
        $rows = $this->phuthu_model->getProductNames($sr);
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $options = $this->phuthu_model->getProductOptions($row->id);
                if ($options) {
                    $opt = $option_id && $r == 0 ? $this->phuthu_model->getProductOptionByID($option_id) : current($options);
                    if (!$option_id || $r > 0) {
                        $option_id = $opt->id;
                    }
                } else {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        $this->phuthu_model->deletePurchase($id);
                    }
                    $this->session->set_flashdata('message', $this->lang->line("phuthu_deleted"));
                    redirect($_SERVER["HTTP_REFERER"]);
                    ...
                    $this->excel->getActiveSheet()->setTitle(lang('phuthu'));
                    ...
                    $row = 2;
                    foreach ($_POST['val'] as $id) {
                        $purchase = $this->phuthu_model->getPurchaseByID($id);
                    ...
                    $filename = 'phuthu_' . date('Y_m_d_H_i_s');
    ...
    public function payments($id = null)
    {
        ...
        $this->data['payments'] = $this->phuthu_model->getPurchasePayments($id);
        $this->data['inv'] = $this->phuthu_model->getPurchaseByID($id);
        $this->load->view($this->theme . 'phuthu/payments', $this->data);
    }

    public function payment_note($id = null)
    {
        $this->sma->checkPermissions('payments', true);
        $payment = $this->phuthu_model->getPaymentByID($id);
        $inv = $this->phuthu_model->getPurchaseByID($payment->purchase_id);
        $this->data['supplier'] = $this->site->getCompanyByID($inv->supplier_id);
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
        ...
        $this->load->view($this->theme . 'phuthu/payment_note', $this->data);
    }
    ...
    public function email_payment($id = null)
    {
        ...
        $html = $this->load->view($this->theme . 'phuthu/payment_note', $this->data, TRUE);
    ...
    public function add_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->phuthu_model->addPayment($payment)) {
            ...
            $this->load->view($this->theme . 'phuthu/add_payment', $this->data);
        }
    }
    ...
    public function edit_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->phuthu_model->updatePayment($id, $payment)) {
            ...
            $this->data['payment'] = $this->phuthu_model->getPaymentByID($id);
            $this->data['modal_js'] = $this->site->modal_js();

            $this->load->view($this->theme . 'phuthu/edit_payment', $this->data);
        }
    }
    ...
    public function delete_payment($id = null)
    {
        ...
        if ($this->phuthu_model->deletePayment($id)) {
    ...
    public function expenses($id = null)
    {
        ...
        $this->page_construct('phuthu/expenses', $meta, $this->data);
    }
    ...
    public function getExpenses()
    {
        ...
        $detail_link = anchor('phuthu/expense_note/$1', '<i class="fa fa-file-text-o"></i> ' . lang('expense_note'), 'data-toggle="modal" data-target="#myModal2"');
        $edit_link = anchor('phuthu/edit_expense/$1', '<i class="fa fa-edit"></i> ' . lang('edit_expense'), 'data-toggle="modal" data-target="#myModal"');
        ...
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('phuthu/delete_expense/$1') . "'>"
        . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
    ...
    {
        $expense = $this->phuthu_model->getExpenseByID($id);
        $this->data['user'] = $this->site->getUser($expense->created_by);
        $this->data['category'] = $expense->category_id ? $this->phuthu_model->getExpenseCategoryByID($expense->category_id) : NULL;
        ...
        $this->load->view($this->theme . 'phuthu/expense_note', $this->data);
    }
    ...
    public function add_expense()
    {
        ...
        if ($this->form_validation->run() == true && $this->phuthu_model->addExpense($data)) {
            ...
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->data['categories'] = $this->phuthu_model->getExpenseCategories();
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'phuthu/add_expense', $this->data);
        }
    }
    ...
    public function edit_expense($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->phuthu_model->updateExpense($id, $data)) {
            $this->session->set_flashdata('message', lang("expense_updated"));
            redirect("phuthu/expenses");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['expense'] = $this->phuthu_model->getExpenseByID($id);
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['categories'] = $this->phuthu_model->getExpenseCategories();
            $this->load->view($this->theme . 'phuthu/edit_expense', $this->data);
        }
    }
    ...
    public function delete_expense($id = null)
    {
        ...
        $expense = $this->phuthu_model->getExpenseByID($id);
        if ($this->phuthu_model->deleteExpense($id)) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    $this->sma->checkPermissions('delete');
                    foreach ($_POST['val'] as $id) {
                        $this->phuthu_model->deleteExpense($id);
                    }
    ...
    {
        $this->sma->checkPermissions('return_phuthu');

        if ($this->input->get('id')) {
            $id = $this->input->get('id');
        }
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $inv = $this->phuthu_model->getReturnByID($id);
        if (!$this->session->userdata('view_right')) {
            $this->sma->view_rights($inv->created_by);
        }
        $this->data['barcode'] = "<img src='" . site_url('products/gen_barcode/' . $inv->reference_no) . "' alt='" . $inv->reference_no . "' class='pull-left' />";
        $this->data['supplier'] = $this->site->getCompanyByID($inv->supplier_id);
        $this->data['payments'] = $this->phuthu_model->getPaymentsForPurchase($id);
        $this->data['user'] = $this->site->getUser($inv->created_by);
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv'] = $inv;
        $this->data['rows'] = $this->phuthu_model->getAllReturnItems($id);
        $this->data['purchase'] = $this->phuthu_model->getPurchaseByID($inv->purchase_id);
        $this->load->view($this->theme.'phuthu/view_return', $this->data);
    }
    ...
    public function return_purchase($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->phuthu_model->addPurchase($data, $products)) {
            ...
            $this->data['reference'] = '';
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('phuthu'), 'page' => lang('phuthu')), array('link' => '#', 'page' => lang('return_purchase')));
            $meta = array('page_title' => lang('return_purchase'), 'bc' => $bc);
            $this->page_construct('phuthu/return_purchase', $meta, $this->data);
        }
    }
    ...
    public function update_status($id)
    {
        ...
        if ($this->form_validation->run() == true && $this->phuthu_model->updateStatus($id, $status, $note)) {
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme.'phuthu/update_status', $this->data);
