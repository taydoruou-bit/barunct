...
class Transfers extends MY_Controller
{
    ...
    function getTransfers()
    {
		...
		$print_link = anchor('transfers/printchuyen/$1', '<i class="fa fa-print"></i> ' . lang('In chuyển kho'), ' class="tip" data-toggle="modal" data-target="#myModal"');
        ...
        $this->datatables
            ->select("id, date, transfer_no,(select name from scodeweb_doitac where id=doitac) as doitac,(select concat(first_name,' ',last_name) FROM scodeweb_users WHERE id={$this->db->dbprefix('transfers')}.created_by) as nhanvien, from_warehouse_name as fname, from_warehouse_code as fcode, to_warehouse_name as tname,to_warehouse_code as tcode, total, total_tax, grand_total, status, attachment")
            ->from('transfers')
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('E1', lang('from_warehouse'));
                    $this->excel->getActiveSheet()->SetCellValue('F1', lang('to_warehouse'));
                    ...
                    foreach ($_POST['val'] as $id) {
						...
						$_dvgh= $this->doitac_model->getDoiTacByID($tansfer->doitac); 
						
						//$warehouse_to=$this->site->getWarehouseByID($tansfer->from_warehouse);	
						//$warehouse_from=$this->site->getWarehouseByID($tansfer->to_warehouse);	
						
						$nhanv=$this->site->getUser($tansfer->created_by);	
						$nhanvien=$nhanv->first_name." ".$nhanv->last_name;
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $dvgh);
						$this->excel->getActiveSheet()->SetCellValue('D' . $row, $nhanvien);
						$this->excel->getActiveSheet()->SetCellValue('E' . $row, $tansfer->from_warehouse_code);
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $tansfer->to_warehouse_code);
	...
	function printchuyen($transfer_id = NULL)
    {
        $this->sma->checkPermissions('index', TRUE);
        ...
        $this->data['rows'] = $rows=$this->transfers_model->getAllTransferItems($transfer_id, $transfer->status);
        $this->data['from_warehouse'] = $from_warehouse=$this->site->getWarehouseByID($transfer->from_warehouse_id);
        $this->data['to_warehouse'] = $to_warehouse=$this->site->getWarehouseByID($transfer->to_warehouse_id);
		$this->data['doitac'] = $doitac=$this->doitac_model->getDoitacByID($transfer->doitac);
        $this->data['transfer'] = $transfer;
        $this->data['tid'] = $transfer_id;
        $this->data['created_by'] = $created_by=$this->site->getUser($transfer->created_by);
        $_tablhd='<table border="1" style="width:100%;border-collapse:collapse;">        <tbody>        <tr>            <td style="text-align:center;width:5%;"><strong>STT</strong>            </td>            <td style="width:51%;padding-left:0.5%;"><strong>Tên hàng hóa</strong><br>            </td>  <td style="text-align:center;width:20%;"><strong>SL</strong><br>            </td> <td style="text-align:center;width:10%;"><strong>Giá</strong>            </td>                     <td style="text-align:center;width:15%;"><strong>T.Tiền</strong><br>            </td>      </tr>';
		if ($this->Settings->tax1) {
			 $_tablhd='<table border="1" style="width:100%;border-collapse:collapse;">        <tbody>        <tr>            <td style="text-align:center;width:5%;"><strong>STT</strong>            </td>            <td style="width:51%;padding-left:0.5%;"><strong>Tên hàng hóa</strong><br>            </td>  <td style="text-align:center;width:10%;"><strong>SL</strong><br>            </td> <td style="text-align:center;width:10%;"><strong>Giá</strong>            </td>   <td style="text-align:center;width:80px;"><strong>Thuế</strong>            </td>                      <td style="text-align:center;width:15%;"><strong>T.Tiền</strong><br>            </td>      </tr>';
		}
		$tong_so=0;
		...
		foreach ($rows as $row){
			...
			$_add_str='<tr><td style="text-align:center;vertical-align:middle;">'.$r.'</td><td style="vertical-align:middle;">'.$_prod_detail.'</td><td style="text-align:center; vertical-align:middle;">'.$this->sma->formatQuantity($row->unit_quantity).'</td><td style="text-align:center; vertical-align:middle;">'.$this->sma->formatQuantity($row->net_unit_cost).'</td>  <td style="text-align:center; vertical-align:middle;">'.$this->sma->formatQuantity($row->subtotal).'</td>    </tr>'; 
			
					
			if ($this->Settings->tax1) {
				$_add_str='<tr><td style="text-align:center;vertical-align:middle;">'.$r.'</td><td style="vertical-align:middle;">'.$_prod_detail.'</td><td style="text-align:center; vertical-align:middle;">'.$this->sma->formatQuantity($row->unit_quantity).'</td><td style="text-align:center; vertical-align:middle;">'.$this->sma->formatQuantity($row->net_unit_cost).'</td><td style="width: 80px; text-align:right; vertical-align:middle;"><!--<small>(' . $row->tax . ')</small>--> ' . $this->sma->formatMoney($row->item_tax) . '</td>  <td style="text-align:center; vertical-align:middle;">'.$this->sma->formatQuantity($row->subtotal).'</td>    </tr>'; 				
			} 
			$_tablhd.=$_add_str;
		...
		$_dc_cuahang_chuyen=str_replace("<p>","",$from_warehouse->address);       
		$_dc_cuahang_chuyen=str_replace("</p>","",$_dc_cuahang_chuyen);
		
		$_dc_cuahang_nhan=str_replace("<p>","",$to_warehouse->address);       
		$_dc_cuahang_nhan=str_replace("</p>","",$_dc_cuahang_nhan);
		
		
		$parse_data = array('So_Hoa_Don' => $transfer->transfer_no,'site_link' => base_url(),'site_name' => $this->Settings->site_name,'Ten_Kho_Chuyen' => $from_warehouse->name,'Dia_Chi_Kho_Chuyen' =>$_dc_cuahang_chuyen,'SDT_Kho_Chuyen' =>$from_warehouse->phone,'Ten_Kho_Nhan' => $to_warehouse->name,'Dia_Chi_Kho_Nhan' =>$_dc_cuahang_nhan,'SDT_Kho_Nhan' =>$to_warehouse->phone,'Ngay_Chuyen' => $this->sma->hrld($transfer->date),'Nhan_Vien' =>$created_by->first_name . ' ' . $created_by->last_name,'Bang_Hoa_Don' =>$_tablhd,'Ghi_Chu' => $this->sma->decode_html($transfer->note),'Tong_Cong' =>$this->sma->formatQuantity($transfer->grand_total),'Tong_So_Luong' =>$this->sma->formatQuantity($tong_so),'Phi_Van_Chuyen' =>$this->sma->formatQuantity($transfer->shipping),'Bang_Chu' =>$_tongcong_bang_chu_text,'Ma_Doi_Tac' => $doitac->code,'Ten_Doi_Tac' => $doitac->name,'Dia_Chi_Doi_Tac' => $doitac->diachi,'Email_Doi_Tac' => $doitac->email,'Dien_Thoai_Doi_Tac' => $doitac->dienthoai,'No_Dau_Ky_Doi_Tac' => $doitac->nodauky);    
		
		if (file_exists('./themes/' . $this->theme . '/views/print_khac/printchuyen.html')) {          
			$sale_temp = file_get_contents('themes/' . $this->theme . '/views/print_khac/printchuyen.html');            
		} else {             
			$sale_temp = file_get_contents('./themes/default/views/print_khac/printchuyen.html');          
		}        
		...
		$this->data['modal_js'] = $this->site->modal_js();  
		$this->load->view($this->theme . 'transfers/printchuyen', $this->data);
    }
	function baocaochuyen()
    {
        $this->sma->checkPermissions();
		...
		$this->data['warehouses'] = $this->site->getAllWarehouses();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('Báo cáo chuyển kho')));
        $meta = array('page_title' => lang('Báo cáo chuyển kho'), 'bc' => $bc);
        $this->page_construct('transfers/indexbaocao', $meta, $this->data);
    }

    function getTransfersBaoCao($pdf = NULL, $xls = NULL)
    {
        $this->sma->checkPermissions('index');
        ...
        if ($pdf || $xls) {
			$this->db->select("date, transfer_no,(select name from scodeweb_doitac where id=doitac) as doitac, (select concat(first_name,' ',last_name) FROM scodeweb_users WHERE id={$this->db->dbprefix('transfers')}.created_by) as nhanvien,from_warehouse_name as fname, to_warehouse_name as tname, total, shipping, grand_total, status,id", false)
            ->from('transfers');
            ...
            if ($reference_no) {
                $this->db->like('transfer_no', $reference_no, 'both');
            }
            if ($to_warehouse) {
                $this->db->where('to_warehouse_id', $to_warehouse);
            }
			if ($from_warehouse) {
                $this->db->where('from_warehouse_id', $from_warehouse);
            }
            if ($user) {
            ...
            if (!empty($data)) {
                ...
                $this->load->library('excel');
                $this->excel->setActiveSheetIndex(0);
                $this->excel->getActiveSheet()->setTitle(lang('Báo cáo chuyển kho'));
                $this->excel->getActiveSheet()->SetCellValue('A1', lang('date'));
                $this->excel->getActiveSheet()->SetCellValue('B1', lang('reference_no'));
                $this->excel->getActiveSheet()->SetCellValue('C1', lang('Nhân viên'));
				$this->excel->getActiveSheet()->SetCellValue('D1', lang('ĐVGH'));
				$this->excel->getActiveSheet()->SetCellValue('E1', lang('Từ kho'));
                $this->excel->getActiveSheet()->SetCellValue('F1', lang('Đến Kho'));
				$this->excel->getActiveSheet()->SetCellValue('G1', lang('Tổng'));
                $this->excel->getActiveSheet()->SetCellValue('H1', lang('Phí VC'));
                $this->excel->getActiveSheet()->SetCellValue('I1', lang('Tổng cộng'));
                ...
                foreach ($data as $data_row) {
					...
					$this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->tname);
                    $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->tname);
					$this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->total);
                    $this->excel->getActiveSheet()->SetCellValue('H' . $row, $data_row->shipping);
			...
			$this->datatables
				->select("date, transfer_no,(select name from scodeweb_doitac where id=doitac) as doitac, (select concat(first_name,' ',last_name) FROM scodeweb_users WHERE id={$this->db->dbprefix('transfers')}.created_by) as nhanvien,from_warehouse_name as fname, to_warehouse_name as tname, total, shipping, grand_total, status,id")
				->from('transfers');
            ...
            if ($reference_no) {
                $this->datatables->like('transfer_no', $reference_no, 'both');
            }
            if ($to_warehouse) {
                $this->datatables->where('to_warehouse_id', $to_warehouse);
            }
			if ($from_warehouse) {
                $this->datatables->where('from_warehouse_id', $from_warehouse);
            }
            if ($user) {
	...
	function baocaochuyenchitiet()
    {
		...
		$this->data['users'] = $this->reports_model->getStaff();
		$this->data['warehouses'] = $this->site->getAllWarehouses();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('Báo cáo chuyển kho chi tiết')));
        $meta = array('page_title' => lang('Báo cáo chuyển kho chi tiết'), 'bc' => $bc);
        $this->page_construct('transfers/indexbaocaochitiet', $meta, $this->data);
    }

    function getTransfersBaoCaoChiTiet($pdf = NULL, $xls = NULL)
    {
        $this->sma->checkPermissions('index');
		$reference_no = $this->input->get('reference_no') ? $this->input->get('reference_no') : NULL;
        $to_warehouse = $this->input->get('to_warehouse') ? $this->input->get('to_warehouse') : NULL;
        ...
        if ($pdf || $xls) {
			...
			if (!$this->Owner && !$this->Admin && !$this->session->userdata('view_right')) {
				$where.=" AND scodeweb_transfers.created_by=".$this->session->userdata('user_id');
			}
			if ($note) {
                $where.=" AND scodeweb_transfers.note LIKE '%".$note."%'";
            }
            if ($reference_no) {
                $where.=" AND scodeweb_transfers.transfer_no LIKE '%".$transfer_no."%'";
            }
            if ($to_warehouse) {
                $where.=" AND to_warehouse_id=".$to_warehouse;
            }
			if ($from_warehouse) {
                $where.=" AND from_warehouse_id=".$from_warehouse;
            }
            if ($user) {
                $where.=" AND scodeweb_transfers.created_by=".$user;
            }
            if ($start_date) {
                $where.=" AND scodeweb_transfers.date BETWEEN '" . $start_date . "' AND '" . $end_date . "'";
            }
			$_sql="SELECT scodeweb_transfers.date, scodeweb_transfers.transfer_no,(select name from scodeweb_doitac where id=doitac) as doitac, (select concat(first_name,' ',last_name) FROM scodeweb_users WHERE id={$this->db->dbprefix('transfers')}.created_by) as nhanvien,from_warehouse_name as fname, to_warehouse_name as tname, scodeweb_purchase_items.product_name as sanpham, scodeweb_purchase_items.product_unit_code as dvt,scodeweb_purchase_items.unit_quantity as sl,scodeweb_purchase_items.unit_cost as giaban, (scodeweb_purchase_items.unit_cost*scodeweb_purchase_items.unit_quantity) as thanhtien, scodeweb_transfers.shipping,scodeweb_transfers.grand_total,scodeweb_transfers.status,scodeweb_transfers.id FROM scodeweb_transfers, scodeweb_purchase_items WHERE scodeweb_transfers.id=scodeweb_purchase_items.transfer_id AND scodeweb_transfers.id>0 $where";
            ...
            if (!empty($data)) {
				...
				$this->excel->getActiveSheet()->SetCellValue('E1', lang('Từ kho'));
                $this->excel->getActiveSheet()->SetCellValue('F1', lang('Đến Kho'));
				$this->excel->getActiveSheet()->SetCellValue('G1', lang('Tên sản phẩm'));
                ...
                $this->excel->getActiveSheet()->SetCellValue('M1', lang('Ship'));
                $this->excel->getActiveSheet()->SetCellValue('N1', lang('Tổng cộng'));

                $row = 2; $total = 0; $sl=0; $tt=0; $shipping=0;
                foreach ($data as $data_row) {
                    $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($data_row->date));
                    $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->transfer_no);
                    $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->nhanvien);
				...
				$this->excel->getActiveSheet()->SetCellValue('M' . $row, $shipping);
                ...
                $filename = 'BaoCao_ChuyenKho_ChiTiet';
                $this->excel->getDefaultStyle()->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                if ($pdf) {
                    $styleArray = array(
                        'borders' => array(
