...
class Sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        // Tính toán chi phí từ custom_fields (fields array)
        // JSON path: $.fields[*] để tìm trong array, dùng JSON_TABLE để extract giá trị
        ...
        $select_query = "scodeweb_sales.id as id, " .
            "DATE_FORMAT(scodeweb_sales.date, '%Y-%m-%d %T') as date, " .
            "reference_no, " .
            "doitac.name as doitac, " .
            "warehouses.name as kho, " .
            "CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ', " . $this->db->dbprefix('users') . ".last_name) as biller, " .
            "CONCAT(scodeweb_sales.customer,'<br/>',scodeweb_companies.name) as customer, " .
            "sale_status, " .
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($select_query)
                ->from('scodeweb_sales')
                ->join('scodeweb_companies', 'scodeweb_companies.id=scodeweb_sales.customer_id', 'left')
                ->join('doitac', 'doitac.id=scodeweb_sales.doitac', 'left')
        ...
        $this->datatables->where('scodeweb_sales.sale_status !=', 'returned');
    ...
    public function add($quote_id = null)
{
    ...
    if ($this->form_validation->run() == true) {
        ...
        // ✅ Lấy deposit_amount từ quote nếu có
        $deposit_amount = 0;
        if ($quote_id) {
            $quote = $this->sales_model->getQuoteByID($quote_id);
            $deposit_amount = $quote && $quote->deposit_amount ? $quote->deposit_amount : 0;
        }
        ...
        // ✅ THÊM: Lấy custom fees từ POST
        $custom_fees = 0;
        if ($this->input->post('custom_fields')) {
            $custom_fields_json = $this->input->post('custom_fields');
            $custom_fields_data = json_decode($custom_fields_json, true);

            if (isset($custom_fields_data['fields']) && is_array($custom_fields_data['fields'])) {
                foreach ($custom_fields_data['fields'] as $field) {
                    if (isset($field['value']) && is_numeric($field['value'])) {
                        $custom_fees += floatval($field['value']);
                    }
                }
            }
        }
        ...
        $data = array(
            ...
            'due_date' => $due_date,
            'paid' => $this->sma->formatDecimal($deposit_amount),
...
// ✅ ƯU TIÊN: Tạo payment từ deposit của quote
if ($deposit_amount > 0) {
    // Cập nhật payment_status
    if ($deposit_amount >= $grand_total) {
        $payment_status = 'paid';
    } else {
    ...
    $payment = array(
        ...
        'amount' => $this->sma->formatDecimal($deposit_amount),
        'paid_by' => 'cash',
        'cheque_no' => '',
        'cc_no' => '',
        'cc_holder' => '',
        ...
        'created_by' => $this->session->userdata('user_id'),
        'warehouse_id' => $warehouse_id,
        'note' => 'Thanh toán đặt cọc từ báo giá #' . $quote_id,
        'type' => 'received',
...
// Trường hợp nhập payment thủ công
elseif (($payment_status == 'partial' || $payment_status == 'paid') && $this->input->post('paid_by')) {
...
// ✅ CẬP NHẬT payment_status vào $data
$data['payment_status'] = $payment_status;
        ...
        // ========== GỌI HÀM INSERT ==========
        if ($this->sales_model->addSale($data, $products, $payment, array(), array(), $dlDetails)) {
            ...
            $this->session->set_flashdata('error', 'Không thể tạo đơn hàng');
            redirect($_SERVER["HTTP_REFERER"]);
        }

    } else {
        // ========== PHẦN LOAD FORM (KHI VALIDATION LỖI HOẶC LẦN ĐẦU VÀO) ==========
        
        if ($quote_id || $sale_id) {
    if ($quote_id) {
        $this->data['quote'] = $this->sales_model->getQuoteByID($quote_id);
        $items = $this->sales_model->getAllQuoteItems($quote_id);
        // ✅ THÊM: Lấy mã tham chiếu từ báo giá
        $this->data['quote_reference_no'] = $this->data['quote']->reference_no;
    } elseif ($sale_id) {
        $this->data['quote'] = $this->sales_model->getInvoiceByID($sale_id);
        $items = $this->sales_model->getAllInvoiceItems($sale_id);
        // ✅ THÊM: Lấy mã tham chiếu từ hóa đơn
        $this->data['quote_reference_no'] = $this->data['quote']->reference_no;
    }
            krsort($items);
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            // Grand total TRỪ custom fees
            $grand_total = $this->sma->formatDecimal(($total + $total_tax + $this->sma->formatDecimal($shipping) - $order_discount), 4);
            $custom_fields_json = $this->input->post('custom_fields');
            $custom_fields_data = null;
            if ($custom_fields_json) {
                $custom_fields_data = $custom_fields_json; // Đã là JSON string từ form
            }
            $data = array(
                'date' => $date,
                'reference_no' => $reference,
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products, $dlDetails)) {
            ...
            // ✅ KHỞI TẠO MẢNG RỖNG TRƯỚC
            $this->data['custom_fields_values'] = array();

            // ✅ LOAD CUSTOM FIELDS TỪ DATABASE
            if (!empty($this->data['inv']->custom_fields)) {
                $custom = json_decode($this->data['inv']->custom_fields);
                if (isset($custom->fields) && is_array($custom->fields)) {
                    foreach ($custom->fields as $field) {
                        $this->data['custom_fields_values'][$field->name] = isset($field->value) ? $field->value : '0';
                    }
                }
            }
    ...
    public function return_sale($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            // Grand total TRỪ custom fees
            $grand_total = $this->sma->formatDecimal(($total + $total_tax + $this->sma->formatDecimal($shipping) - $order_discount - $custom_fees), 4);
            $data = array(
                'date' => $date,
                'sale_id' => $id,
    ...
    public function add_gift_card_qua()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'expiry' => $this->input->post('expiry') ? $this->sma->fsd($this->input->post('expiry')) : null,
                'created_by' => $this->session->userdata('user_id'),
                'tenquatang' => $this->input->post('tenquatang'),
                'type' => 'qua',
                'ca_points' => $this->input->post('ca_points')
            );
            $sa_data = array();
    ...
    public function getThuHoi($warehouse_id = null)
    {
        ...
        $pdf_link = anchor('sales/pdf/$1', '<i class="fa fa-file-pdf-o"></i> ' . lang('download_pdf'));
        $return_link = anchor('sales/return_sale/$1', '<i class="fa fa-angle-double-left"></i> ' . lang('return_sale'));
        $print_link = anchor('sales/printhoadon/$1', '<i class="fa fa-print"></i> ' . lang('print_hoadon'), 'data-toggle="modal" data-target="#myModal"');
        $delete_link = "<a href='#' class='po' title='<b>" . lang("delete_sale") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('sales/delete/$1') . "'>"
            . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
            . lang('delete_sale') . "</a>";
        $action = '<div class="text-center"><div class="btn-group text-left">'
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $pr = array(
                    'id' => ($c + $r),
                    'item_id' => $row->id,
                    'label' => '[KHUYẾN MÃI] ' . $row->name . " (" . $row->code . ")",
                    'category' => $row->category_id,
                    'row' => $row,
                    'combo_items' => $combo_items,
                    'tax_rate' => $tax_rate,
